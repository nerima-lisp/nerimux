...
(defmacro with-fake-two-pane-session ((var) &body body)
  ...
  `(with-fake-session (,var :nwindows 1 :npanes 2) ,@body))

(defmacro with-minimal-session ((pane-var win-var
                                          sess-var
                                          &key
                                          (width 20)
                                          (height 5)) &body
    ...
    `(let* ((,w-sym ,width)
            ...
            (,win-var
             (make-window :id
                          ...
                          (list ,pane-var)
                          :tree
                          (make-layout-leaf ,pane-var)))
            (,sess-var (make-session :id 1 :name "s" :windows (list ,win-var))))
