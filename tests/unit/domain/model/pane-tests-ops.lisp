...
(describe "model-suite"
    ...
    (let* ((call nil)
      ...
      (let ((nerimux/model:*pane-extra-env* '(("GLOBAL" . "value"))))
        (multiple-value-bind (fd pid tty)
          ...
          (expect (null nerimux/model:*pane-extra-env*)))
  ...
  (it "fork-pane-builds-a-pane-from-the-pty-result"
    (let ((nerimux/ports:*spawn-pty*
      ...
      (let ((pane (nerimux/pane::%fork-pane
        ...
        (expect (string= "/dev/pts/fork" (nerimux/model:pane-tty pane)))
        (expect (string= "" (nerimux/model:pane-start-command pane)))
        (expect (string= "/tmp/fork" (nerimux/model:pane-start-path pane)))
  ...
  (it "make-input-pane-uses-dead-pty-sentinels"
    (let ((pane (nerimux/pane::%make-input-pane 4 5 6 30 7)))
      ...
      (expect (string= "" (nerimux/model:pane-tty pane)))
      (expect (nerimux/model:pane-screen pane))))
  ...
  (it "respawn-pane-closes-old-pty-and-resets-pane-state"
    (let* ((screen (make-screen 40 6))
      ...
      (expect (string= "/dev/pts/new" (nerimux/model:pane-tty pane)))
      (expect (string= "printf ok" (nerimux/model:pane-start-command pane)))
      (expect (string= "/new" (nerimux/model:pane-start-path pane)))
      (expect (eq screen (nerimux/model:pane-screen pane)))
      (expect (null (nerimux/model:pane-unread-output-p pane)))
      (expect (null (nerimux/model:pane-bell-p pane)))
      (expect (null (nerimux/model:pane-process-exited-p pane)))
      (expect (null (nerimux/model:pane-non-zero-exit-p pane)))
      (expect (null (nerimux/model:pane-startup-failed-p pane)))))
  )
