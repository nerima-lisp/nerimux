  ...
  (it "uses the main worktree for repository ahead and behind state"
    (let* ((repository (nerimux/workspace-model:make-repository
           ...
           (main-worktree (nerimux/workspace-model:make-worktree
                           ...
                           :ahead 3
                           :behind 4))
           (feature-worktree (nerimux/workspace-model:make-worktree
      ...
      (expect (eq main-worktree (nerimux/workspace-model:repository-main-worktree repository)))
      (expect (= 3 (nerimux/workspace-model:repository-ahead repository)))
      (expect (= 4 (nerimux/workspace-model:repository-behind repository)))
...
(describe "repository-values"
  (it "keeps the raw model constructor defaults available"
    (let ((repository (nerimux/workspace-model::%make-repository)))
      (expect (equal "" (nerimux/workspace-model:repository-id repository)))
      (expect (equal "" (nerimux/workspace-model:repository-specification repository)))
      (expect (equal "" (nerimux/workspace-model:repository-local-path repository)))
      (expect (eq :git (nerimux/workspace-model:repository-backend repository)))
      (expect (null (nerimux/workspace-model:repository-worktrees repository)))
      (expect (= 0 (nerimux/workspace-model:repository-ahead repository)))
      (expect (= 0 (nerimux/workspace-model:repository-behind repository)))))
  ...
  (it "normalizes identifiers and copies worktree collections"
    (let ((worktrees (list (nerimux/workspace-model:make-worktree :path "/work/one"))))
      (let ((repository (nerimux/workspace-model:make-repository
                         :specification "nerima-lisp/nerimux"
