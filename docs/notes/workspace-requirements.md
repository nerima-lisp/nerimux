...
| 項目 | 決定 |
| --- | --- |
| split ratio の resize | **再検討・実装済み（2026-09-07）。** FR-105。pane 分割層を UI に配線し、真のマルチプレクサとして完成させる |
| サーバ再起動をまたぐ永続化 | **再検討・実装済み（2026-09-07）。** FR-206 |
...
| mouse | **完全に撤去する** |
| エージェント発の通知 | **再検討・実装済み（2026-09-07）。** FR-104 と FR-201/202。monitor-activity / bell / silence、visual-bell は全廃 |
