...
> **Historical document.** This roadmap records the coverage-closing process.
> For current behavior, see the [architecture](../src/reference/architecture.md)
> and [security model](../src/reference/security-model.md).

## 1. Executive Summary

**Historical scope:** the old 12–15% estimate is no longer used. The items below record previously tracked blockers and their current status.
...
1. **Parity checks only:** the previously tracked blockers are implemented or intentionally unsupported. Record newly discovered tmux behavior differences here.
2. **Behavioral parity audits:** audit edge cases where an implementation path exists but tmux semantics may still differ.
...
**Notes:** `38;5;n` and `38;2;r;g;b` are parsed and rendered. Re-check tmux parity for edge-case color sequences that are not yet mirrored.
...
**Priority:** implemented; the files and tests below cover target resolution.
...
**Priority:** implemented; the files and tests below cover session registry behavior.
...
`switch-client` are no longer wired through any dispatch command table. The
`src/application/dispatch/` directory that held that wiring was deleted along with
...
**Notes:** `select-pane -L/-R/-U/-D` routes through directional neighbor lookup, and prefix arrow-key bindings are wired. Re-check tmux edge cases.
...
**Priority:** implemented; the files and tests below cover status-bar formatting and message expansion.
...
**Notes:** `copy-mode-select-word`, `copy-mode-yank`, and the paste-buffer plumbing already exist; audit their tmux parity here.
...
**Notes:** startup auto-forwarding and `ensure-server-running` are wired. Re-check tmux parity as needed.
...
the rest of the tmux command table, so no tmux command name resolves from any
...
- **Since removed, partially.** `bind-key`/`unbind-key` still parse and
  populate `*key-tables*`. The historical compatibility audit's relevant section
  says the only remaining reader of that table is a `list-keys`-style formatter;
  nothing calls it from a live entry point. The keystroke pipeline that used to
...
unaffected. It was on the historical compatibility audit's "Still effective" list.
...
**Since removed.** `src/presentation/events/events-mouse.lisp` is the file
every bullet above cites. It no longer exists, and (unlike copy mode) mouse
...
**Since removed, partially.** The hooks system, `confirm-before`, `wait-for`,
`display-popup`, `display-menu`, and `key-tables`'s "keystroke dispatch
files" all depended on `src/application/dispatch/`, which is now gone.
...
- **control mode (tmux -C)**: implemented in `src/infrastructure/control-mode/control-mode.lisp`, `src/bootstrap/main.lisp`, and `src/application/dispatch/dispatch-control.lisp`; notifications are emitted as `%begin`/`%end`-delimited blocks and covered by the control-mode unit tests.
  **Since removed.** This audit entry records the former implementation. The
  capability and the three files above are no longer in the tree; see `git log
  -- src/infrastructure/control-mode/` and the historical compatibility audit's
  “Removed” section.
...
**Partially removed.** Enforcement moved, but nothing sets the flag any more.
The `events-*`/`dispatch-*` files this bullet cites are
...
`src/bootstrap/runtime.lisp` stays permanently `nil` today. See the historical
compatibility audit's relevant section and the fuller writeup in
`docs/notes/permissions-and-verification.md`.

### Additional terminal emulation
- **Line drawing / ACS**: ESC `(0` / ESC `(B` character set switching and ACS remapping are implemented in `src/domain/terminal/parser.lisp`, `src/domain/terminal/modes.lisp`, and `src/domain/terminal/cursor.lisp`. New coverage and documentation should match those files.
...
The format pipeline is split across `src/domain/format/format-helpers.lisp`, `src/domain/format/format-strftime.lisp`, `src/domain/format/format.lisp`, `src/domain/format/format-engine.lisp`, and `src/domain/format/format-context.lisp`. `nerimux.asd` loads those files before the renderer and dispatch layers. `format-context-from-session` builds the plist consumed by `expand-format` from session, window, and pane state.

### Multi-session server and protocol framing are already target-aware
`src/infrastructure/net/protocol.lisp` encodes and decodes target-bearing commands, and `src/bootstrap/server-multi.lisp` reconstructs the command line with `-t <target>` before dispatch. New wire messages must carry their target for multi-session routing.
...
The key-table code remains in `src/application/config/config.lisp` and
`src/application/config/config-directives.lisp`, but the runtime resolution path
was removed with `src/presentation/events/`. Parsing and storage remain covered
by the Sprint 2 config work.
...
`render-session-to-string` is documented as pure. Features such as zoom overlays, copy-mode highlighting, display-panes numbering, and popup windows must store overlay state in the model (window/pane structs), not in the renderer. The renderer reads state; it does not mutate it.
