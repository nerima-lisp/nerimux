...
6. workspaceを一覧に追加し、起動対象の選択画面を開く。
7. `Agent` を選べばagent選択画面へ進み、`Terminal` を選べば通常terminalを1つ起動する。
8. 起動対象の選択をキャンセルした場合は、作成したworkspaceを削除する。

利用者の入力はrepository選択と起動対象（Agent / Terminal）の選択だけとする。fetch中も一覧、既存workspace、terminalは操作可能である。fetch失敗時はworkspaceを作成せず、エラーと再試行操作を表示する。

起動対象の選択はworkspace作成直後に一度だけ表示する。`Terminal` はagentを割り当てずに利用できるため、調査・手動編集・短時間の確認を目的とするworkspaceを最短操作で作成できる。後から `a` でagent、`t` で追加terminalを割り当てられる。
...
- Claudeは `claude --dangerously-skip-permissions` で起動する。
- Codexは `codex --dangerously-bypass-approvals-and-sandbox` で起動する。
...
- workspace作成直後の起動対象で `Terminal` を選ぶと、agentなしの通常terminalを起動する。
- 通常terminalはシェルを起動し、agent用の特別な初期コマンドやプロンプトを注入しない。
...
workspace overview
  ...
  ├─ n     → workspace作成 → Agent / Terminal選択
  ├─ a     → agent割り当て
  ├─ t     → terminal追加
  ├─ c     → 完了 / 未完了切替
  ├─ p     → 選択workspaceをprune
...
- workspace作成時にagentなしの通常terminalを選択して起動できる。
- agent 1つ、terminal複数の制約が守られる。
- Claude / Codexがそれぞれ指定された起動コマンドで起動される。
