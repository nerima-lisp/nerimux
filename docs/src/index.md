...
There is no standalone in-process mode. `nerimux attach` and `nerimux server`
are the only entry points. A hermetic regression suite pins documented behavior;
...
value the workspace UI depends on, including shell, `$TERM`, scrollback length,
split ratios, and pane limits, is a compiled-in constant.
