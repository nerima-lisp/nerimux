...
(defmacro with-fresh-options (&body body)
  ...
  `(let ((nerimux/options:*global-options*   (make-hash-table :test #'equal))
         (nerimux/options:*option-registry*  (make-hash-table :test #'equal)))
     ,@body))
...
(defmacro with-fresh-server-options (&body body)
  ...
  `(let ((nerimux/options:*server-options* (make-hash-table :test #'equal))
         (nerimux/options:*server-option-registry* nerimux/options:*server-option-registry*))
     ,@body))
...
(defmacro with-single-server-option ((name value) &body body)
  ...
  `(let ((nerimux/options:*server-options*
          (let ((ht (make-hash-table :test #'equal)))
            (setf (gethash ,name ht) ,value)
            ht)))
     ,@body))
  ...
  (let ((actual-sym (gensym "ACTUAL")))
    `(progn
       ...
       (let ((,actual-sym ,(if server-p
                               `(nerimux/options:get-server-option ,option nil)
                               `(nerimux/options:get-option ,option nil))))
         ,(if present-p
