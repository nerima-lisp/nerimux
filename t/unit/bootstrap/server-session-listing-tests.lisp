...
(describe "server-suite"
  ...
  (it "list-sessions-format"
    (with-empty-registry
      (let ((s1 (make-session :id 1 :name "mysession" :windows (list 'w1 'w2))))
        (setf nerimux::*server-sessions* (list (cons "mysession" s1)))
        (let ((*overlay* nil)
              (nerimux::*dirty* nil)
              (nerimux::*running* t))
          (nerimux::dispatch-command s1 :list-sessions nil)
  ...
  (it "rename-session-updates-registry"
    (with-empty-registry
      (with-fake-session (s :nwindows 1)
        ...
        (expect (eq s (nerimux::server-find-session "renamed-sess")))
        (expect (null (nerimux::server-find-session "0"))))))
  ...
  (it "switch-client-prev-touches-prev-session"
    (with-empty-registry
      (let ((s1 (make-session :id 1 :name "s1" :windows nil :last-active 100))
        ...
        (nerimux::server-add-session s2)
        (with-loop-state
          (nerimux::dispatch-command s2 :switch-client-prev nil)
          (expect nerimux::*dirty* :to-be-truthy)))))
  ...
  (it "last-session-cycles-by-recency"
    (with-empty-registry
      (let ((s1 (make-session :id 1 :name "oldest" :windows nil :last-active 100))
        ...
        (let ((old-active-s3 (nerimux/model:session-last-active s3))
              (nerimux::*dirty* nil)
              (nerimux::*running* t))
          (nerimux::dispatch-command s2 :last-session nil)
          (expect (> (nerimux/model:session-last-active s3) old-active-s3))
          (expect nerimux::*dirty* :to-be-truthy)))))
  ...
  (it "display-message-sets-overlay"
    (with-fake-session (s :nwindows 1)
      (let ((*overlay* nil) (*prompt* nil)
            (nerimux::*dirty* nil) (nerimux::*running* t))
  ...
  (it "source-file-prompts-for-path"
    (with-fake-session (s :nwindows 1)
      (let ((*prompt* nil))
        (nerimux::dispatch-command s :source-file nil)
        (expect (prompt-active-p))
