...
(describe "main-suite"
  ...
  ;; %socket-file-session-name extracts the session name from a nerimux socket path.
  (it "socket-file-session-name-extracts-from-valid-path"
    (dolist (row '(("/tmp/nerimux-0.sock"    "0"    "default session name")
                   ("/tmp/nerimux-work.sock"  "work"  "named session")
                   ("/tmp/nerimux-my-s.sock"  "my-s"  "hyphenated name")))
      (destructuring-bind (path expected desc) row
        (declare (ignore desc))
        (expect (string= expected (nerimux::%socket-file-session-name path))))))

  ;; %socket-file-session-name returns NIL for paths without the nerimux- prefix.
  (it "socket-file-session-name-returns-nil-for-non-nerimux-path"
    (expect (null (nerimux::%socket-file-session-name "/tmp/other-program.sock")))
    (expect (null (nerimux::%socket-file-session-name nil))))
  ...
  (it "socket-file-session-name-returns-nil-for-empty-name"
    ...
    (expect (null (nerimux::%socket-file-session-name ""))))
  ...
  (it "list-commands-arguments-parses-format-flag"
    (multiple-value-bind (fmt name)
        (nerimux::%list-commands-arguments '("-F" "#{command_list_name}"))
  ...
  (it "list-commands-arguments-parses-positional-name"
    (multiple-value-bind (fmt name)
        (nerimux::%list-commands-arguments '("new-session"))
      (expect (null fmt))
  ...
  (it "list-commands-arguments-parses-both-flags"
    (multiple-value-bind (fmt name)
        (nerimux::%list-commands-arguments '("-F" "#{name}" "kill-server"))
  ...
  (it "list-commands-arguments-returns-nil-nil-for-empty-list"
    (multiple-value-bind (fmt name)
        (nerimux::%list-commands-arguments '())
      (expect (null fmt))
      (expect (null name))))
  ...
  (it "define-forwarding-commands-generates-defun"
    (let* ((expansion (macroexpand-1
                       '(nerimux::define-forwarding-commands
  ...
  (it "run-list-commands-is-fbound"
    (expect (fboundp 'nerimux::run-list-commands)))
  ...
  (it "parse-new-session-flags-parses-name-flag"
    (multiple-value-bind (name _win _detach _dir)
        (nerimux::%parse-new-session-flags '("-s" "mywork"))
  ...
  (it "parse-new-session-flags-parses-detach-flag"
    (multiple-value-bind (_name _win detach _dir)
        (nerimux::%parse-new-session-flags '("-d"))
  ...
  (it "parse-new-session-flags-parses-start-dir"
    (multiple-value-bind (_name _win _detach dir)
        (nerimux::%parse-new-session-flags '("-c" "/tmp/work"))
  ...
  (it "parse-new-session-flags-returns-nils-for-empty-args"
    (multiple-value-bind (name win detach dir)
        (nerimux::%parse-new-session-flags '())
      (expect (null name))
      (expect (null win))
      (expect (null detach))
      (expect (null dir)))))
