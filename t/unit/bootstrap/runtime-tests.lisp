...
(describe "runtime-suite"
  ...
  (it "runtime-max-message-log-entries-is-constant"
    (expect (constantp 'nerimux::+max-message-log-entries+))
    (expect (integerp nerimux::+max-message-log-entries+))
    (expect (plusp nerimux::+max-message-log-entries+)))
  ...
  (it "reader-reading-state-honors-window-local-monitor-activity"
    (with-isolated-state
      ...
      (let* ((sess        (make-fake-session :nwindows 2))
             (active-win  (nerimux/model:session-active-window sess))
             (bg-win      (find-if-not (lambda (w) (eq w active-win))
                                       (nerimux/model:session-windows sess))))
        (expect (not (null bg-win)))
        (nerimux/options:set-option "monitor-activity" nil)              ; global = NIL
        ;; Window-local "on" on the background window.
        (nerimux/options:set-option-for-window "monitor-activity" "on" bg-win)
        (expect (eq t (nerimux/options:get-option-for-context "monitor-activity" :window bg-win)))
        ;; The active window has no local override → resolves to global NIL.
        (expect (null (nerimux/options:get-option-for-context "monitor-activity" :window active-win))))))
  ...
  (it "reader-reading-state-window-local-monitor-activity-off-over-global-on"
    (with-isolated-state
      (let* ((sess       (make-fake-session :nwindows 2))
             ...
             (bg-win     (find-if-not (lambda (w) (eq w active-win))
                                      (nerimux/model:session-windows sess))))
        (nerimux/options:set-option "monitor-activity" t)               ; global = T
        (nerimux/options:set-option-for-window "monitor-activity" "off" bg-win) ; window = NIL
        (expect (null (nerimux/options:get-option-for-context "monitor-activity" :window bg-win))))))
  ...
  (it "mark-window-activity-fires-alert-activity-hook"
    (with-isolated-state
      (let* ((sess  (make-fake-session :nwindows 1))
             ...
             (fired nil))
        (nerimux/options:set-option "monitor-activity" "on")
        (setf (nerimux/model:window-activity-flag win) nil)
        (nerimux/hooks:add-hook "alert-activity"
                                (lambda (&rest _) (declare (ignore _)) (setf fired t)))
        (nerimux::%mark-window-activity win)
        (expect (nerimux/model:window-activity-flag win) :to-be-truthy)
  ...
  (it "mark-window-bell-sets-sticky-flag-and-fires-hook"
    (with-isolated-state
      (let* ((sess     (make-fake-session :nwindows 1))
        ...
        (nerimux::%mark-window-bell win pane)
        (expect (nerimux/model:window-bell-flag win) :to-be-truthy)
      ...
      (destructuring-bind (monitor action pending expect-flag expect-hook desc) row
        ...
        (with-isolated-state
          (let* ((sess  (make-fake-session :nwindows 1))
                 ...
                 (pane  (first (nerimux/model:window-panes win)))
                 (fired nil))
            (nerimux/options:set-option "monitor-bell" monitor)
            (nerimux/options:set-option "bell-action" action)
            (setf (nerimux/terminal/types:screen-bell-pending
                   (nerimux/model:pane-screen pane)) pending)
            (nerimux/hooks:add-hook "alert-bell"
                                    (lambda (&rest _) (declare (ignore _)) (setf fired t)))
            (nerimux::%mark-window-bell win pane)
            (expect (eq expect-flag (nerimux/model:window-bell-flag win)))
  ...
  (it "mark-window-bell-current-window-is-noop"
    (with-isolated-state
      (let* ((sess (make-fake-session :nwindows 1))
             (win  (nerimux/model:session-active-window sess))
             (pane (first (nerimux/model:window-panes win)))
             (nerimux::*server-sessions* (list (cons 1 sess))))
        (setf (nerimux/terminal/types:screen-bell-pending
               (nerimux/model:pane-screen pane)) t)
        (nerimux::%mark-window-bell win pane)
        (expect (nerimux/model:window-bell-flag win) :to-be-falsy))))
      ...
      (destructuring-bind (visual expect-overlay) row
        (with-isolated-state
          (let* ((sess (make-fake-session :nwindows 1))
            ...
            (nerimux/options:set-option "visual-bell" visual)
            (setf (nerimux/terminal/types:screen-bell-pending
                   (nerimux/model:pane-screen pane)) t)
            (nerimux::%mark-window-bell win pane)
            (if expect-overlay
                (expect (search "Bell in window" (or nerimux/prompt:*overlay* "")))
                (expect (null nerimux/prompt:*overlay*))))))))
  ...
  (it "session-select-window-clears-bell-flag"
    (let* ((sess (make-fake-session :nwindows 2))
           (win  (second (nerimux/model:session-windows sess))))
      (setf (nerimux/model:window-bell-flag win) t)
      (nerimux/model:session-select-window sess win)
      (expect (nerimux/model:window-bell-flag win) :to-be-falsy)))
  ...
  (it "monitor-silence-fires-alert-silence-hook"
    (with-isolated-state
      (let* ((sess  (make-fake-session :nwindows 1))
             (win   (nerimux/model:session-active-window sess))
             (fired nil))
        (nerimux/options:set-option "monitor-silence" 5)
        ...
        (nerimux/options:set-option "silence-action" "any")
        (setf (nerimux/model:window-last-output-time win) (- (get-universal-time) 100)
              (nerimux/model:window-silence-flag win) nil)
        (nerimux/hooks:add-hook "alert-silence"
                                (lambda (&rest _) (declare (ignore _)) (setf fired t)))
        (nerimux::%check-monitor-silence (list (cons 1 sess)) (lambda () nil))
  ...
  (it "monitor-silence-default-is-zero-no-op"
    (with-isolated-state
      (let* ((sess (make-fake-session :nwindows 1))
             (win  (nerimux/model:session-active-window sess)))
        (expect (eql 0 (nerimux/options:get-option "monitor-silence")))
  ...
  (it "monitor-silence-visual-silence-shows-overlay"
    (with-isolated-state
      (let* ((sess (make-fake-session :nwindows 1))
             ...
             (nerimux/prompt:*overlay* nil))
        (nerimux/options:set-option "monitor-silence" 5)
        (nerimux/options:set-option "silence-action" "any")  ; fire for the current window
        (nerimux/options:set-option "visual-silence" "on")
        (setf (nerimux/model:window-last-output-time win) (- (get-universal-time) 100)
              (nerimux/model:window-silence-flag win) nil)
        (nerimux::%check-monitor-silence (list (cons 1 sess)) (lambda () nil))
  ...
  (it "alert-action-fires-p-policy-matrix"
    (expect (nerimux::%alert-action-fires-p "none" t) :to-be-falsy)
    (expect (nerimux::%alert-action-fires-p "none" nil) :to-be-falsy)
    (expect (nerimux::%alert-action-fires-p "current" t) :to-be-truthy)
    (expect (nerimux::%alert-action-fires-p "current" nil) :to-be-falsy)
    (expect (nerimux::%alert-action-fires-p "any" t) :to-be-truthy)
    (expect (nerimux::%alert-action-fires-p "any" nil) :to-be-truthy)
    (expect (nerimux::%alert-action-fires-p "other" t) :to-be-falsy)
    (expect (nerimux::%alert-action-fires-p "other" nil) :to-be-truthy))
  ...
  (it "silence-action-none-suppresses-alert"
    (with-isolated-state
      (let* ((sess (make-fake-session :nwindows 1))
             ...
             (fired nil))
        (nerimux/options:set-option "monitor-silence" 5)
        (nerimux/options:set-option "silence-action" "none")
        (setf (nerimux/model:window-last-output-time win) (- (get-universal-time) 100)
              (nerimux/model:window-silence-flag win) nil)
        (nerimux/hooks:add-hook "alert-silence"
                                (lambda (&rest _) (declare (ignore _)) (setf fired t)))
