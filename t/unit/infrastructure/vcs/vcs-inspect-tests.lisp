...
;;;; grep across t/ finds no helper anywhere that shells out to `git init`,
    ...
    (let ((worktree (nerimux/model:make-worktree
      ...
      (with-stubbed-fdefinition
        ...
        (let ((thread
          ...
          (expect (null (nerimux/model:worktree-commits-state worktree)))
          (expect (= 1 (length queued)))
          (funcall (pop queued))
          (expect (eq :ready (nerimux/model:worktree-commits-state worktree)))
    ...
    (let ((worktree (nerimux/model:make-worktree
      ...
      (with-stubbed-fdefinition
        ...
        (let ((thread
          ...
          (cl-concurrent-kit:join-thread thread :timeout 2)
          (expect (eq :failed (nerimux/model:worktree-commits-state worktree)))
          (expect (null (nerimux/model:worktree-recent-commits worktree))))))))
    ...
    (let* ((previous (nerimux/vcs:workspace-organizations))
      ...
      (unwind-protect
          (progn
            ...
            (nerimux/model:repository-add-worktree repository struct-a)
            ...
            (with-stubbed-fdefinition
              ...
              (let ((thread
                ...
                (nerimux/model:repository-add-worktree repository struct-b)
                (cl-concurrent-kit:join-thread thread :timeout 2)
                (expect (= 1 (length queued)))
                (funcall (pop queued))
                (expect (eq :ready (nerimux/model:worktree-commits-state struct-b)))
                (expect (equal (list (cons "abc1234" "settled onto b"))
                               (nerimux/model:worktree-recent-commits struct-b)))
                (expect (null (nerimux/model:worktree-commits-state struct-a)))
