...
;;;; Tests for nerimux/buffer: paste-buffer ring operations.
;;;;
...
(describe "buffer-suite"
  ...
  (it "osc52-inbound-respects-set-clipboard-off"
    (with-empty-buffers
      (with-fresh-options
        (nerimux/options:set-option "set-clipboard" "off")
        (nerimux/buffer::%osc52-inbound-clipboard "secret")
  ...
  (it "osc52-inbound-accepted-when-set-clipboard-on"
    (with-empty-buffers
      (with-fresh-options
        (nerimux/options:set-option "set-clipboard" "on")
        (nerimux/buffer::%osc52-inbound-clipboard "copied")
        (expect (string= "copied" (nerimux/buffer:get-paste-buffer 0))))))

  ;; Most recently added buffer is returned as index 0.
  (it "buffer-lifo-order"
    (with-empty-buffers
      (nerimux/buffer:add-paste-buffer "first")
      (nerimux/buffer:add-paste-buffer "second")
      (check-table
       (list (list (nerimux/buffer:get-paste-buffer 0) "second"
                   "index 0 must be most recently added")
             (list (nerimux/buffer:get-paste-buffer 1) "first"
  ...
  (it "buffer-clear-empties-ring"
    (with-empty-buffers
      ...
      (expect (null (nerimux/buffer:list-paste-buffers)))))

  ;; list-paste-buffers returns a fresh list, not the internal one.
  (it "buffer-list-returns-copy"
    (with-empty-buffers
      (nerimux/buffer:add-paste-buffer "x")
      (let ((lst (nerimux/buffer:list-paste-buffers)))
        (setf (car lst) "mutated")
        (expect (string= "x" (nerimux/buffer:get-paste-buffer 0))))))
  ...
  (it "buffer-limit-enforced-via-option"
    (with-empty-buffers
      ...
      (let ((saved (nerimux/options:get-option "buffer-limit")))
        (nerimux/options:set-option "buffer-limit" 3)
        (unwind-protect
             (progn
               (nerimux/buffer:add-paste-buffer "a")
               (nerimux/buffer:add-paste-buffer "b")
               (nerimux/buffer:add-paste-buffer "c")
               (nerimux/buffer:add-paste-buffer "d")  ; should evict "a"
               (expect (= 3 (length (nerimux/buffer:list-paste-buffers))))
               (check-table
                (loop for (idx expected desc)
                        in '((0 "d" "index 0 must be \"d\"")
                             (1 "c" "index 1 must be \"c\"")
                             (2 "b" "index 2 must be \"b\""))
                      collect (list (nerimux/buffer:get-paste-buffer idx)
                                    expected
                                    desc))
                :test #'string=)
               (expect (null (nerimux/buffer:get-paste-buffer 3))))
          (nerimux/options:set-option "buffer-limit" saved)))))

  ;; Without a configured limit, at most +default-buffer-limit+ (50) entries are kept.
  (it "buffer-limit-default-50"
    (with-empty-buffers
      (let ((limit nerimux/buffer:+default-buffer-limit+))
        (dotimes (i (+ limit 5))
          (nerimux/buffer:add-paste-buffer (format nil "buf~D" i)))
        (expect (<= (length (nerimux/buffer:list-paste-buffers)) limit)))))
  ...
  (it "buffer-delete-middle-index"
    (with-empty-buffers
      (nerimux/buffer:add-paste-buffer "a")
      (nerimux/buffer:add-paste-buffer "b")
      (nerimux/buffer:add-paste-buffer "c")  ; c=0, b=1, a=2
      (nerimux/buffer:delete-paste-buffer 1)  ; remove b
      (check-table
       (list (list (length (nerimux/buffer:list-paste-buffers)) 2
                   "ring length after deleting middle entry"))
       :test #'=)
      (check-table
       (list (list (nerimux/buffer:get-paste-buffer 0) "c"
                   "newest entry remains at index 0")
             (list (nerimux/buffer:get-paste-buffer 1) "a"
  ...
  (it "buffer-add-returns-text"
    (with-empty-buffers
      (expect (string= "hello" (nerimux/buffer:add-paste-buffer "hello")))))
  ...
  (it "buffer-limit-fallback-when-options-error"
    ...
    (let ((empty-ht (make-hash-table :test #'equal)))
      (let ((nerimux/options:*global-options* empty-ht))
        ...
        (expect (= nerimux/buffer:+default-buffer-limit+ (nerimux/buffer::%buffer-limit))))))
    ...
    (dolist (entry
      ...
      (destructuring-bind (text desc) entry
        ...
        (with-empty-buffers
          ...
          (expect (string= text (nerimux/buffer:get-paste-buffer 0)))))))

  ;;; ── +default-buffer-limit+ constant ─────────────────────────────────────────

  ;; +default-buffer-limit+ is a positive integer.
  (it "default-buffer-limit-is-positive-integer"
    (expect (and (integerp nerimux/buffer:+default-buffer-limit+)
                 (plusp nerimux/buffer:+default-buffer-limit+))))
  ...
  (it "list-paste-buffers-most-recent-first"
    (with-empty-buffers
      (nerimux/buffer:add-paste-buffer "first")
      (nerimux/buffer:add-paste-buffer "second")
      (nerimux/buffer:add-paste-buffer "third")
      (let ((lst (nerimux/buffer:list-paste-buffers)))
        (check-table
         (loop for (idx expected desc)
  ...
  (it "delete-paste-buffer-last-entry-empties-ring"
    (with-empty-buffers
      (nerimux/buffer:add-paste-buffer "only-one")
      (nerimux/buffer:delete-paste-buffer 0)
      (expect (null (nerimux/buffer:list-paste-buffers)))))
  ...
  (it "clear-paste-buffers-idempotent-on-empty-ring"
    (with-empty-buffers
      (finishes (nerimux/buffer:clear-paste-buffers))
      (expect (null (nerimux/buffer:list-paste-buffers)))))
  ...
  (it "osc52-handler-is-set-to-add-paste-buffer"
    (expect nerimux/terminal/parser:*osc52-handler* :to-be-truthy)
    (expect (functionp nerimux/terminal/parser:*osc52-handler*)))

  ;; Calling *osc52-handler* with text pushes it onto the paste buffer ring.
  (it "osc52-handler-populates-paste-buffer"
    (with-empty-buffers
      (funcall nerimux/terminal/parser:*osc52-handler* "clipboard-text")
      (expect (string= "clipboard-text" (nerimux/buffer:get-paste-buffer 0)))))
    ...
    (with-empty-buffers
      (let ((s (make-screen 10 5)))
        ...
        (let ((seq (cl-codec-kit:string-to-octets
                    ...
                    :encoding :latin-1)))
          (nerimux/terminal/emulator:screen-process-bytes s seq))
