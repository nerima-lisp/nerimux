...
(describe "buffer-suite"
  ...
  (it "base64-encode-known-value"
    (expect (string= "aGVsbG8="
                 (nerimux/terminal/parser::%base64-encode
  ...
  (it "base64-encode-decode-round-trip"
    (let* ((bytes (cl-codec-kit:string-to-octets "Round-trip 123! αβγ" :encoding :utf-8))
           (round (nerimux/terminal/parser::%base64-decode
                   (nerimux/terminal/parser::%base64-encode bytes))))
  ...
  (it "osc52-clipboard-sequence-format"
    (let ((seq (nerimux/terminal/parser:osc52-clipboard-sequence "hi")))
