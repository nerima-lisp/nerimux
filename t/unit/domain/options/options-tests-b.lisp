...
(describe "options-suite"
  ...
  (it "define-option-accessor-macro-is-defined"
    (expect (macro-function 'nerimux/options::define-option-accessor)))

  ;;; ── define-type-coercions macro ──────────────────────────────────────────

  ;; define-type-coercions is a registered macro.
  (it "define-type-coercions-macro-is-defined"
    (expect (macro-function 'nerimux/options::define-type-coercions)))
  ...
  (it "coerce-value-table-driven"
    (dolist (entry '(;; :boolean branch
      ...
      (destructuring-bind (type input expected) entry
        (let ((result (nerimux/options::%coerce-value type input)))
  ...
  (it "show-option-server-scope-absent"
    (with-fresh-server-options
      (let ((out (nerimux/options:show-option "nonexistent-server-opt" :server)))
  ...
  (it "integer-coercion-non-numeric-falls-back-to-zero"
    (expect (= 0 (nerimux/options::%coerce-value :integer t)))
    (expect (= 0 (nerimux/options::%coerce-value :integer :foo))))
  ...
  (it "server-option-registry-is-hash-table"
    (expect (hash-table-p nerimux/options:*server-option-registry*))
    (dolist (name '("escape-time" "exit-empty" "exit-unattached"))
      (expect (not (null (gethash name nerimux/options:*server-option-registry*))))))
  ...
  (it "server-options-exit-unattached-default"
    (expect (null (nerimux/options:get-server-option "exit-unattached"))))
    ...
    (let ((win (nerimux/model:make-window :id 1 :name "test-win"))
          (nerimux/options:*global-options*
           (let ((ht (make-hash-table :test #'equal)))
             ...
             ht)))
      (expect (= 9999 (nerimux/options:get-option-for-window "history-limit" win)))))
  ...
  (it "get-option-for-window-falls-back-to-spec-default"
    (let ((win (nerimux/model:make-window :id 1 :name "test-win"))
      ...
      (expect (string= "on" (nerimux/options:get-option-for-window "status" win)))))
    ...
    (let ((p (nerimux/model:make-pane :id 1))
          (nerimux/options:*global-options*
           (let ((ht (make-hash-table :test #'equal)))
             ...
             ht)))
      (expect (= 7777 (nerimux/options:get-option-for-pane "history-limit" p)))))
  ...
  (it "get-option-for-window-falsey-local-overrides-truthy-global"
    (with-isolated-config
      (nerimux/options:set-option "synchronize-panes" t)
      (let ((win (make-fake-window 1 "w")))
        (nerimux/options:set-option-for-window "synchronize-panes" "off" win)
        (expect (null (nerimux/options:get-option-for-window "synchronize-panes" win)))
  ...
  (it "get-option-for-pane-falsey-local-overrides-truthy-global"
    (with-isolated-config
      ...
      (let* ((win  (make-fake-window 1 "w"))
             (pane (first (nerimux/model:window-panes win))))
        (nerimux/options:set-option-for-pane "remain-on-exit" "off" pane)
        (expect (null (nerimux/options:get-option-for-pane "remain-on-exit" pane)))
  ...
  (it "set-option-for-window-coerces-integer"
    (with-isolated-config
      (let ((win (make-fake-window 1 "w")))
        (nerimux/options:set-option-for-window "history-limit" "5000" win)
        (expect (eql 5000 (nerimux/options:get-option-for-window "history-limit" win))))))
  ...
  (it "get-option-for-context-skips-nil-levels"
    (with-isolated-config
      ...
      (expect (eq (nerimux/options:get-option "synchronize-panes")
                  (nerimux/options:get-option-for-context "synchronize-panes")))
  ...
  (it "get-option-for-context-falls-back-to-registry-default"
    (with-isolated-config
      (let* ((win  (make-fake-window 1 "w" :npanes 1))
             (pane (first (nerimux/model:window-panes win))))
        (expect (eql 2000 (nerimux/options:get-option-for-context
  ...
  (it "get-option-for-context-pane-falsey-honored-over-window"
    (with-isolated-config
      (nerimux/options:set-option "synchronize-panes" t)      ; global = T
      (let* ((win  (make-fake-window 1 "w" :npanes 1))
             (pane (first (nerimux/model:window-panes win))))
        (nerimux/options:set-option-for-window "synchronize-panes" "on" win)  ; window = T
        (nerimux/options:set-option-for-pane   "synchronize-panes" "off" pane) ; pane = NIL
        (expect (null (nerimux/options:get-option-for-context
                       "synchronize-panes" :pane pane :window win)))
  ...
  (it "get-option-for-context-window-falsey-over-global"
    (with-isolated-config
      (nerimux/options:set-option "synchronize-panes" t)            ; global = T
      (let ((win (make-fake-window 1 "w" :npanes 1)))
        (nerimux/options:set-option-for-window "synchronize-panes" "off" win)  ; window = NIL
        (expect (null (nerimux/options:get-option-for-context
                       "synchronize-panes" :window win)))
  ...
  (it "get-option-for-context-global-falsey-over-default"
    (with-isolated-config
      (nerimux/options:set-option "history-limit" 1)               ; global differs from default 2000
      (let* ((win  (make-fake-window 1 "w" :npanes 1))
             (pane (first (nerimux/model:window-panes win))))
        (expect (eql 1 (nerimux/options:get-option-for-context
                        "history-limit" :pane pane :window win))))))
  ...
  (it "append-option-value-style-vs-plain"
    (dolist (c '(("status-style" "bg=red" "fg=blue" "bg=red,fg=blue" "non-empty style -> comma separator")
      ...
      (destructuring-bind (name old new expected desc) c
        (declare (ignore desc))
        (expect (string= expected (nerimux/options:append-option-value name old new)))))))
