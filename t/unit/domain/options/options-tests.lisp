...
;;;; Tests for nerimux/options: option registry and get/set/coercion.
;;;;
     ...
     ,@(mapcar
        (lambda (entry)
          (destructuring-bind (name check) entry
            (cond
              ((eq check :registered)
               `(expect (nerimux/options:option-defined-p ,name)))
              ((eq check :string-p)
               `(progn
                  (expect (nerimux/options:option-defined-p ,name))
                  (expect (stringp (nerimux/options:get-option ,name)))))
              (t
               `(expect (equal ,check (nerimux/options:get-option ,name)))))))
        entries)))
...
(describe "options-suite"
  ...
  (it "get-option-unset-reverts-to-registry-default"
    ...
    (with-fresh-global-options
      (remhash "default-terminal" nerimux/options:*global-options*)
      (expect (string= "screen" (nerimux/options:get-option "default-terminal")))
      ;; A registered option with no caller default also falls back even if never set.
      (remhash "history-limit" nerimux/options:*global-options*)
      (expect (= 2000 (nerimux/options:get-option "history-limit")))
      ;; An explicitly supplied default (even NIL) is still honored over the registry.
      (expect (null (nerimux/options:get-option "default-terminal" nil)))
      (expect (= 7 (nerimux/options:get-option "default-terminal" 7)))
      ;; An UNregistered key still returns the caller default / NIL (no spec to fall back to).
      (expect (null (nerimux/options:get-option "totally-unknown-opt")))))
  ...
  (it "get-server-option-unset-reverts-to-registry-default"
    (with-fresh-server-options
      ...
      (expect (string= "screen" (nerimux/options:get-server-option "default-terminal")))
      ...
      (expect (null (nerimux/options:get-server-option "default-terminal" nil)))
  ...
  (it "set-and-get-option-string"
    (with-fresh-options
      (nerimux/options:set-option "status-left" "my-session")
      (expect (string= "my-session" (nerimux/options:get-option "status-left")))))
  ...
  (it "option-defined-p-table"
    (dolist (row '(("status"         t   "known option → T")
      ...
      (destructuring-bind (name expected desc) row
        (declare (ignore desc))
        (expect (if expected
                (nerimux/options:option-defined-p name)
                (null (nerimux/options:option-defined-p name)))))))
  ...
  (it "status-numeric-value-survives-coercion"
    (with-fresh-global-options
      (expect (string= "2" (nerimux/options:set-option "status" "2")))
      (expect (string= "2" (nerimux/options:get-option "status")))
      (expect (string= "off" (nerimux/options:set-option "status" "off")))
      (expect (string= "on" (nerimux/options:set-option "status" "on")))))
  ...
  (it "server-options-exit-empty-default"
    (expect (nerimux/options:get-server-option "exit-empty")))

  ;; set-server-option stores a value in *server-options*.
  (it "set-server-option-stores-value"
    (with-fresh-server-options
      (nerimux/options:set-server-option "escape-time" "100")
      (expect (= 100 (nerimux/options:get-server-option "escape-time")))))

  ;; set-server-option coerces boolean values.
  (it "set-server-option-boolean-coercion"
    (with-fresh-server-options
      (nerimux/options:set-server-option "exit-empty" "off")
  ...
  (it "show-option-missing-option"
    (with-fresh-options
      (let ((out (nerimux/options:show-option "no-such-option")))
  ...
  (it "show-options-server-scope"
    (with-single-server-option ("escape-time" 500)
      (let ((out (nerimux/options:show-options :server)))
  ...
  (it "update-environment-default"
    (let ((val (nerimux/options:get-option "update-environment")))
      (expect (stringp val))
  ...
  (it "set-option-boolean-set-titles"
    (with-fresh-global-options
      (expect (eq t (nerimux/options:set-option "set-titles" "on")))
      (expect (eq t (nerimux/options:get-option "set-titles")))
      (expect (null (nerimux/options:set-option "set-titles" "off")))
      (expect (null (nerimux/options:get-option "set-titles")))))

  ;; status-left-length integer set/get round-trip.
  (it "set-option-integer-status-left-length"
    (with-fresh-global-options
      (expect (= 80 (nerimux/options:set-option "status-left-length" "80")))
      (expect (= 80 (nerimux/options:get-option "status-left-length")))))

  ;; pane-base-index set/get round-trip.
  (it "pane-base-index-set-get"
    (with-fresh-global-options
      (expect (= 1 (nerimux/options:set-option "pane-base-index" "1")))
      (expect (= 1 (nerimux/options:get-option "pane-base-index")))))
  ...
  (it "option-scope-from-name-window-options"
    (dolist (name '("automatic-rename" "mode-keys" "mode-style"
      ...
      (expect (eq :window (nerimux/options:option-scope-from-name name)))))
  ...
  (it "option-scope-from-name-session-options"
    (dolist (name '("status" "status-interval" "history-limit"
      ...
      (expect (eq :session (nerimux/options:option-scope-from-name name))))))
