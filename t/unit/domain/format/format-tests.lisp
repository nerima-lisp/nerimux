...
(describe "format-suite"
  ...
  (defun fmt (template &rest ctx-pairs)
    ...
    (nerimux/format:expand-format template (apply #'list ctx-pairs)))
  ...
  (it "format-W-modifier-iterates-windows"
    (with-isolated-config
      (let* ((sess (make-fake-session :nwindows 3))
        ...
        (expect (string= "X X X" (nerimux/format:expand-format "#{W:X}" ctx)))
        ;; Per-window context: each window's own index is expanded — 3 single-digit
        ;; indices joined by 2 separator spaces.
        (let ((out (nerimux/format:expand-format "#{W:#{window_index}}" ctx)))
  ...
  (it "format-W-modifier-active-vs-inactive"
    (with-isolated-config
      (let* ((sess (make-fake-session :nwindows 3))
             ...
             (out  (nerimux/format:expand-format "#{W:A,I}" ctx)))
  ...
  (it "format-W-modifier-no-session-is-empty"
    (expect (string= "" (nerimux/format:expand-format
                         "#{W:X}" (nerimux/format:format-context-from-session nil nil nil)))))
  ...
  (it "format-P-modifier-iterates-panes"
    (with-isolated-config
      (let* ((sess (make-fake-session :nwindows 1 :npanes 3))
             ...
             (pane (first (nerimux/model:window-panes win)))
             (ctx  (nerimux/format:format-context-from-session sess win pane)))
        (expect (string= "XXX" (nerimux/format:expand-format "#{P:X}" ctx)))
        (expect (= 3 (length (nerimux/format:expand-format "#{P:#{pane_index}}" ctx)))))))
  ...
  (it "format-P-modifier-active-vs-inactive"
    (with-isolated-config
      (let* ((sess (make-fake-session :nwindows 1 :npanes 3))
             (win  (nerimux/model:session-active-window sess))
             (ctx  (nerimux/format:format-context-from-session sess win nil))
             (out  (nerimux/format:expand-format "#{P:A,I}" ctx)))
        (expect (= 1 (count #\A out)))
        (expect (= 2 (count #\I out))))))
  ...
  (it "format-P-modifier-no-window-is-empty"
    (expect (string= "" (nerimux/format:expand-format
                         "#{P:X}" (nerimux/format:format-context-from-session nil nil nil)))))
  ...
  (it "format-S-modifier-iterates-server-sessions"
    (with-isolated-config
      (let* ((s1  (make-fake-session :nwindows 1))
             ...
             (nerimux::*server-sessions* (list (cons "a" s1) (cons "b" s2)))
             (ctx (nerimux/format:format-context-from-session
                   s1 (nerimux/model:session-active-window s1) nil)))
        (expect (string= "XX" (nerimux/format:expand-format "#{S:X}" ctx)))
        (let ((out (nerimux/format:expand-format "#{S:A,I}" ctx)))
          (expect (= 1 (count #\A out)))
  ...
  (it "format-context-window-index-matches-window-id"
    (let* ((sess (make-fake-session :nwindows 2))
           ...
           (win1 (first wins))
           (win2 (second wins))
           (pane (first (nerimux/model:window-panes win1))))
      (let ((ctx1 (nerimux/format:format-context-from-session sess win1 pane)))
        (expect (= (nerimux/model:window-id win1) (getf ctx1 :window-index))))
      (let* ((pane2 (first (nerimux/model:window-panes win2)))
             (ctx2  (nerimux/format:format-context-from-session sess win2 pane2)))
        (expect (= (nerimux/model:window-id win2) (getf ctx2 :window-index))))))
  ...
  (it "format-window-raw-flags-active-window-has-star"
    (with-format-context (sess win pane ctx) ()
      (let ((raw   (nerimux/format:expand-format "#{window_raw_flags}" ctx))
            (flags (nerimux/format:expand-format "#{window_flags}" ctx)))
        (expect (search "*" raw))
        (expect (search "*" flags)))))
  ...
  (it "format-window-raw-flags-inactive-empty-vs-flags-space"
    (let* ((sess (make-fake-session :nwindows 2))
           ...
           (pane     (first (nerimux/model:window-panes inactive)))
           (ctx      (nerimux/format:format-context-from-session sess inactive pane)))
      (let ((raw   (nerimux/format:expand-format "#{window_raw_flags}" ctx))
            (flags (nerimux/format:expand-format "#{window_flags}" ctx)))
        (expect (string= "" raw))
        (expect (string= " " flags)))))
  ...
  (it "format-window-raw-flags-zoomed-window-has-z"
    (let* ((sess (make-fake-session :nwindows 1))
      ...
      (let* ((ctx (nerimux/format:format-context-from-session sess win pane))
             (raw (nerimux/format:expand-format "#{window_raw_flags}" ctx)))
        (expect (search "*" raw))
        (expect (search "Z" raw)))))
  ...
  (it "format-context-pane-index-matches-pane-id"
    (let* ((sess  (make-fake-session :nwindows 1 :npanes 2))
           ...
           (ctx1  (nerimux/format:format-context-from-session sess win (first panes)))
           (ctx2  (nerimux/format:format-context-from-session sess win (second panes))))
      (expect (= (nerimux/model:pane-id (first panes)) (getf ctx1 :pane-index)))
      (expect (= (nerimux/model:pane-id (second panes)) (getf ctx2 :pane-index)))))

  ;; format-context-from-session propagates :session-name and :window-name correctly.
  (it "format-context-names-propagated"
    (with-format-context (sess win pane ctx) ()
      (expect (string= (nerimux/model:session-name sess) (getf ctx :session-name)))
      (expect (string= (nerimux/model:window-name win) (getf ctx :window-name)))))
  ...
  (it "expand-format-uses-window-count-from-context"
    (with-format-context (sess win pane ctx) (:nwindows 4)
      (expect (string= "4" (nerimux/format:expand-format "#{window_count}" ctx)))))
  ...
  (it "format-context-window-active-table"
    (let* ((sess (make-fake-session :nwindows 2))
           ...
           (win1 (first wins))
           (win2 (second wins)))
      (expect (string= "1" (getf (nerimux/format:format-context-from-session
                                   sess win1 (first (nerimux/model:window-panes win1)))
                                  :window-active)))
      (expect (string= "0" (getf (nerimux/format:format-context-from-session
                                   sess win2 (first (nerimux/model:window-panes win2)))
                                  :window-active)))))
  ...
  (it "format-context-window-flags-table"
    (let* ((sess (make-fake-session :nwindows 2))
           ...
           (win1 (first wins))
           (win2 (second wins)))
      (expect (string= "*" (getf (nerimux/format:format-context-from-session
                                   sess win1 (first (nerimux/model:window-panes win1)))
                                  :window-flags)))
      (expect (string= " " (getf (nerimux/format:format-context-from-session
                                   sess win2 (first (nerimux/model:window-panes win2)))
  ...
  (it "expand-format-time-expands"
    (with-format-context (sess win pane ctx) ()
      (let ((result (nerimux/format:expand-format "#{time}" ctx)))
  ...
  (it "format-context-from-window-works"
    (let* ((sess (make-fake-session :nwindows 2))
           (win  (first (nerimux/model:session-windows sess)))
           (ctx  (nerimux/format:format-context-from-window sess win)))
  ...
  (it "variable-to-keyword-table"
    (dolist (c '(("session_name" :session-name "session_name → :session-name")
      ...
      (destructuring-bind (input expected desc) c
        (declare (ignore desc))
        (expect (eq expected (nerimux/format::%variable-to-keyword input))))))
    ...
    (dolist (c '(("1,yes,no"  "1"         "yes" "no"  "both branches")
      ...
      (destructuring-bind (input ec et ef desc) c
        ...
        (multiple-value-bind (cond-str true-str false-str)
            (nerimux/format::%split-conditional input)
  ...
  (it "lookup-returns-empty-for-missing-key"
    (expect (string= "" (nerimux/format::%lookup '() :missing-key))))
    ...
    (let ((ctx (list :count 42 :label "hello")))
      (dolist (c '((:count "42") (:label "hello")))
        (destructuring-bind (key expected) c
          (expect (string= expected (nerimux/format::%lookup ctx key)))))))
  ...
  (it "short-hostname-table"
    (dolist (c '(("myhost.example.com" "myhost" "FQDN → short hostname")
      ...
      (destructuring-bind (input expected desc) c
        (declare (ignore desc))
        (expect (string= expected (nerimux/format::%short-hostname input)))))))
