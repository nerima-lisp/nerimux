...
(describe "format-suite"
  ...
  (it "format-context-cursor-character-empty-when-pane-nil"
    (let ((ctx (nerimux/format:format-context-from-session nil nil nil)))
      (expect (string= "" (nerimux/format:expand-format "#{cursor_character}" ctx)))))
  ...
  (it "format-context-cursor-character-reads-glyph-under-cursor"
    (let* ((sess (make-fake-session :nwindows 1 :npanes 1))
      ...
      (setf (nerimux/terminal/types:screen-cell scr 2 1)
            (nerimux/terminal/types:make-cell :char #\Z))
      (setf (nerimux/terminal/types:screen-cursor-x scr) 2
            (nerimux/terminal/types:screen-cursor-y scr) 1)
      (let ((ctx (nerimux/format:format-context-from-session sess win pane)))
        (expect (string= "Z" (nerimux/format:expand-format "#{cursor_character}" ctx))))))
  ...
  (it "format-context-pane-in-mode-in-copy-mode"
    (let* ((sess (make-fake-session :nwindows 1 :npanes 1))
           ...
           (pane (first (nerimux/model:window-panes win)))
           (scr  (nerimux/model:pane-screen pane)))
      (setf (nerimux/terminal/types:screen-copy-mode-p scr) t)
      (let ((ctx (nerimux/format:format-context-from-session sess win pane)))
        (expect (string= "1" (getf ctx :pane-in-mode))))
      (setf (nerimux/terminal/types:screen-copy-mode-p scr) nil)))
  ...
  (it "format-context-copy-cursor-reports-position-in-copy-mode"
    (let* ((sess (make-fake-session :nwindows 1 :npanes 1))
           ...
           (scr  (nerimux/model:pane-screen pane)))
      (setf (nerimux/terminal/types:screen-copy-mode-p scr) t
            (nerimux/terminal/types:screen-copy-cursor scr) (cons 7 3)
            (nerimux/terminal/types:screen-copy-selecting scr) t)
      (let ((ctx (nerimux/format:format-context-from-session sess win pane)))
        (dolist (c '(("#{copy_cursor_x}"    "3" "copy cursor column (cdr of row.col)")
          ...
          (destructuring-bind (spec expected desc) c
            ...
            (expect (string= expected (nerimux/format:expand-format spec ctx))))))
      (setf (nerimux/terminal/types:screen-copy-mode-p scr) nil
            (nerimux/terminal/types:screen-copy-selecting scr) nil)))
  ...
  (it "format-context-copy-position-empty-outside-copy-mode"
    (let* ((sess (make-fake-session :nwindows 1 :npanes 1))
      ...
      (dolist (spec '("#{copy_position}" "#{copy_position_limit}"))
        (expect (string= "" (nerimux/format:expand-format spec ctx))))))
  ...
  (it "format-context-copy-position-and-limit-in-copy-mode"
    (let* ((sess (make-fake-session :nwindows 1 :npanes 1))
      ...
      (setf (nerimux/terminal/types:screen-copy-mode-p scr) t
            (nerimux/terminal/types:screen-copy-offset scr) 3)
      (let ((ctx (nerimux/format:format-context-from-session sess win pane)))
        (expect (string= "3" (nerimux/format:expand-format "#{copy_position}" ctx)))
        (expect (string= "5" (nerimux/format:expand-format "#{copy_position_limit}" ctx))))
      (setf (nerimux/terminal/types:screen-copy-mode-p scr) nil)))
  ...
  (it "format-context-window-last-flag-correct"
    (let* ((sess (make-fake-session :nwindows 2))
           (win0 (first  (nerimux/model:session-windows sess)))
           (win1 (second (nerimux/model:session-windows sess))))
      ;; Simulate: win0 was active first (lower time), win1 is now active (higher time).
      (setf (nerimux/model:window-last-active-time win0) 100
            (nerimux/model:window-last-active-time win1) 200)
      (nerimux/model:session-select-window sess win1)
      (let ((ctx0 (nerimux/format:format-context-from-session sess win0 nil))
            (ctx1 (nerimux/format:format-context-from-session sess win1 nil)))
  ...
  (it "format-expand-session-id-and-window-id"
    (let* ((sess (make-fake-session :nwindows 1 :npanes 1))
           ...
           (pane (first (nerimux/model:window-panes win)))
           (ctx  (nerimux/format:format-context-from-session sess win pane)))
      (expect (plusp (length (nerimux/format:expand-format "#{session_id}" ctx))))
      (expect (plusp (length (nerimux/format:expand-format "#{window_id}" ctx)))))))
