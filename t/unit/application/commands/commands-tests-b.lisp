...
(describe "commands-suite"
  ...
  (it "copy-mode-line-end-moves-to-last-content-column"
    ...
    (let ((s (copy-mode-screen :content "abc")))
      ...
      (nerimux/commands::copy-mode-line-end s)
    ...
    (let ((s (copy-mode-screen :content "abc")))
      ...
      (nerimux/commands::copy-mode-line-end s)
      (expect (= (1- (nerimux/terminal/types:screen-width s))
                 (cdr (nerimux/terminal/types:screen-copy-cursor s))))))
  ...
  (it "copy-mode-line-start-end-noop-table"
    (dolist (c '((nerimux/commands::copy-mode-line-start 10 "line-start: col unchanged")
                 (nerimux/commands::copy-mode-line-end    3  "line-end: col unchanged")))
      (destructuring-bind (fn col desc) c
        (declare (ignore desc))
        (let ((s (make-screen 20 5)))
          (setf (nerimux/terminal/types:screen-copy-cursor s) (cons 0 col))
          (funcall fn s)
          (expect (= col (cdr (nerimux/terminal/types:screen-copy-cursor s))))))))
  ...
  (it "copy-mode-h-m-l-moves-table"
    (dolist (c '((nerimux/commands::copy-mode-high   (7 . 5) 0 "high → row 0")
                 (nerimux/commands::copy-mode-middle  (0 . 5) 5 "middle → floor(10/2)=5")
                 (nerimux/commands::copy-mode-low     (0 . 5) 9 "low → height-1=9")))
      (destructuring-bind (fn init-cursor expected-row desc) c
        (declare (ignore desc))
        (let ((s (make-screen 20 10)))
          (nerimux/commands::copy-mode-enter s)
          (setf (nerimux/terminal/types:screen-copy-cursor s) init-cursor)
          (funcall fn s)
          (expect (= expected-row (car (nerimux/terminal/types:screen-copy-cursor s))))
  ...
  (it "copy-mode-h-m-l-noop-outside-copy-mode-table"
    (dolist (c '((nerimux/commands::copy-mode-high   3 "high: row unchanged")
                 (nerimux/commands::copy-mode-middle  7 "middle: row unchanged")
                 (nerimux/commands::copy-mode-low     2 "low: row unchanged")))
      (destructuring-bind (fn row desc) c
        (declare (ignore desc))
        (let ((s (make-screen 20 10)))
          (setf (nerimux/terminal/types:screen-copy-cursor s) (cons row 5))
          (funcall fn s)
          (expect (= row (car (nerimux/terminal/types:screen-copy-cursor s))))))))

  ;; copy-mode-cursor-centre-vertical/horizontal set the centre row/column.
  (it "copy-mode-cursor-centre-moves-table"
    (dolist (c '((nerimux/commands::copy-mode-cursor-centre-vertical
                  (2 . 7) 5 7 "vertical centre → row 5")
                 (nerimux/commands::copy-mode-cursor-centre-horizontal
      ...
      (destructuring-bind (fn init-cursor expected-value expected-keep desc) c
        ...
        (let ((s (make-screen 20 10)))
          ...
          (funcall fn s)
          (ecase fn
            (nerimux/commands::copy-mode-cursor-centre-vertical
             (expect (= expected-value (car (nerimux/terminal/types:screen-copy-cursor s))))
             (expect (= expected-keep (cdr (nerimux/terminal/types:screen-copy-cursor s)))))
            (nerimux/commands::copy-mode-cursor-centre-horizontal
             (expect (= expected-value (cdr (nerimux/terminal/types:screen-copy-cursor s))))
             (expect (= expected-keep (car (nerimux/terminal/types:screen-copy-cursor s))))))))))
  ...
  (it "copy-mode-cursor-centre-noop-outside-copy-mode-table"
    (dolist (c '((nerimux/commands::copy-mode-cursor-centre-vertical 3 5 "vertical: row unchanged")
                 (nerimux/commands::copy-mode-cursor-centre-horizontal 7 2 "horizontal: col unchanged")))
      (destructuring-bind (fn row col desc) c
        (declare (ignore desc))
        (let ((s (make-screen 20 10)))
          (setf (nerimux/terminal/types:screen-copy-cursor s) (cons row col))
          (funcall fn s)
          (expect (= row (car (nerimux/terminal/types:screen-copy-cursor s))))
          (expect (= col (cdr (nerimux/terminal/types:screen-copy-cursor s))))))))
  ...
  (it "copy-mode-page-up-scrolls-by-full-height"
    (let ((s (%screen-with-scrollback 30)))
      (nerimux/commands::copy-mode-page-up s)
  ...
  (it "copy-mode-page-down-scrolls-forward-by-full-height"
    (let ((s (%screen-with-scrollback 30)))
      ...
      (nerimux/commands::copy-mode-page-down s)
  ...
  (it "copy-mode-half-page-up-scrolls-by-half-height"
    (let ((s (%screen-with-scrollback 30)))
      (nerimux/commands::copy-mode-half-page-up s)
  ...
  (it "copy-mode-scroll-up-line-scrolls-by-one"
    (let ((s (%screen-with-scrollback 10)))
      (nerimux/commands::copy-mode-scroll-up-line s)
  ...
  (it "copy-mode-scroll-down-line-scrolls-forward-by-one"
    (let ((s (%screen-with-scrollback 10)))
      ...
      (nerimux/commands::copy-mode-scroll-down-line s)
  ...
  (it "copy-mode-scroll-up-noop-outside-copy-mode-table"
    (dolist (fn '(nerimux/commands::copy-mode-page-up
                  nerimux/commands::copy-mode-half-page-up
                  nerimux/commands::copy-mode-scroll-up-line
                  nerimux/commands::copy-mode-top))
      (let ((s (make-screen 20 5)))
  ...
  (it "copy-mode-scroll-down-noop-outside-copy-mode-table"
    (dolist (fn '(nerimux/commands::copy-mode-page-down
                  nerimux/commands::copy-mode-half-page-down
                  nerimux/commands::copy-mode-scroll-down-line
                  nerimux/commands::copy-mode-bottom))
      (let ((s (make-screen 20 5)))
        (setf (nerimux/terminal/types:screen-copy-offset s) 0)
        (funcall fn s)
        (expect (= 0 (screen-copy-offset s))))))
  ...
  (it "copy-mode-word-motion-noop-outside-copy-mode-table"
    (dolist (fn '(nerimux/commands::copy-mode-word-forward
                  nerimux/commands::copy-mode-word-end
                  nerimux/commands::copy-mode-word-backward))
      (let ((s (make-screen 20 5)))
        (feed s "hello world")
        (setf (nerimux/terminal/types:screen-copy-cursor s) (cons 0 0))
        (funcall fn s)
  ...
  (it "copy-mode-word-end-jumps-to-end-of-word"
    (let ((s (copy-mode-screen :w 40 :content "hello world")))
      ;; Cursor at col 0 (start of "hello")
      (setf (nerimux/terminal/types:screen-copy-cursor s) (cons 0 0))
      (nerimux/commands::copy-mode-word-end s)
  ...
  (it "copy-mode-top-jumps-to-max-scrollback"
    (let ((s (%screen-with-scrollback 10)))
      (nerimux/commands::copy-mode-top s)
  ...
  (it "copy-mode-bottom-returns-to-live-view"
    (let ((s (%screen-with-scrollback 10)))
      ...
      (expect (= 10 (screen-copy-offset s)))
      ;; Then jump to bottom
      (nerimux/commands::copy-mode-bottom s)
      (expect (= 0 (screen-copy-offset s))))))
