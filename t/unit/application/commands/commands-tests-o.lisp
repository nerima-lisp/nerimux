...
(describe "commands-suite"
  ...
  (it "copy-mode-word-backward-wraps-to-prev-row"
    ...
    (let ((s (make-screen 10 2)))
      (feed s (format nil "hello~C~Cworld" #\Return #\Linefeed))
      (nerimux/commands::copy-mode-enter s)
      (setf (nerimux/terminal/types:screen-copy-cursor s) (cons 1 0))
      (nerimux/commands::copy-mode-word-backward s)
  ...
  (it "copy-mode-paragraph-jumps-to-blank-line-table"
    ...
    (dolist (row (list (list (cons 0 0)
                             #'nerimux/commands::copy-mode-next-paragraph
                             "next-paragraph from row 0 → blank row 2")
                       (list (cons 4 0)
                             #'nerimux/commands::copy-mode-previous-paragraph
      ...
      (destructuring-bind (start-cursor fn desc) row
        ...
        (let ((s (make-screen 20 5)))
          (feed s (format nil "hello~C~Cworld~C~C~C~Cfoo~C~Cbar" #\Return #\Linefeed
                          ...
                          #\Return #\Linefeed))
          (nerimux/commands::copy-mode-enter s)
          (setf (nerimux/terminal/types:screen-copy-cursor s) start-cursor)
          (funcall fn s)
          (expect (= 2 (car (nerimux/terminal/types:screen-copy-cursor s))))))))
  ...
  (it "copy-mode-next-paragraph-at-bottom-stays"
    (let ((s (make-screen 20 3)))
      ...
      (nerimux/commands::copy-mode-enter s)
      (setf (nerimux/terminal/types:screen-copy-cursor s) (cons 0 0))
      (nerimux/commands::copy-mode-next-paragraph s)
  ...
  (it "copy-mode-row-blank-p-distinguishes-blank-and-content-rows"
    (let ((s (copy-mode-screen :w 8 :h 3 :content "ab")))
      (expect (nerimux/commands::%copy-mode-row-blank-p s 0) :to-be-falsy)
      (expect (nerimux/commands::%copy-mode-row-blank-p s 1) :to-be-truthy)))
  ...
  (it "find-paragraph-boundary-scans-to-nearest-blank-row"
    (let ((s (make-screen 20 5)))
      ...
      (expect (= 2 (nerimux/commands::%find-paragraph-boundary s 1 :down 5)))
      (expect (= 2 (nerimux/commands::%find-paragraph-boundary s 3 :up 0)))
      (expect (= 4 (nerimux/commands::%find-paragraph-boundary s 4 :down 5)))))
  ...
  (it "copy-mode-scroll-middle-centers-cursor"
    ...
    (let ((s (make-screen 20 5)))
      ...
      (nerimux/commands::copy-mode-enter s)
      (nerimux/commands::copy-mode-top s)   ; scroll to max offset (3)
      (let ((max-off (screen-copy-offset s)))
        (setf (nerimux/terminal/types:screen-copy-cursor s) (cons 4 0))
        (nerimux/commands::copy-mode-scroll-middle s)
        (expect (= 2 (car (nerimux/terminal/types:screen-copy-cursor s))))
  ...
  (it "copy-mode-jump-to-mark-moves-cursor-to-mark"
    (let* ((s (make-screen 20 5)))
      (setf (screen-copy-mode-p s)  t
            ...
            (nerimux/terminal/types:screen-copy-cursor s) (cons 4 10)
            (nerimux/terminal/types:screen-copy-mark   s) (cons 1 3)
            (nerimux/terminal/types:screen-copy-mark-offset s) 0)
      (nerimux/commands::copy-mode-jump-to-mark s)
      (expect (equal (cons 1 3) (nerimux/terminal/types:screen-copy-cursor s)))))
