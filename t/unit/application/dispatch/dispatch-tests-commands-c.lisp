...
(describe "dispatch-suite"
  ...
  (it "toggle-synchronize-panes-table"
    (dolist (row '((nil "ON"  "toggling from off must show ON")
      ...
      (destructuring-bind (initial expected-text desc) row
        ...
        (with-loop-state
          (let ((*overlay* nil))
            (nerimux/options:set-option "synchronize-panes" initial)
            (nerimux::%toggle-synchronize-panes)
  ...
  (it "cyclic-navigation-table"
    (dolist (c '((nerimux::next-cyclic (x)       x       x  "next of only element → itself")
                 (nerimux::next-cyclic (x)       missing x  "next with unknown current → element")
                 (nerimux::prev-cyclic (x)       x       x  "prev of only element → itself")
                 (nerimux::next-cyclic (a b c d) b       c  "next from middle → following")
                 (nerimux::prev-cyclic (a b c d) b       a  "prev from middle → preceding")))
  ...
  (it "with-active-window-macro-is-defined"
    (expect (macro-function 'nerimux::with-active-window)))
  ...
  (it "copy-mode-cmd-returns-override-for-known-char"
    (expect (eq :copy-mode-exit (nerimux::%copy-mode-cmd #\q)))
    (expect (eq :copy-mode-exit (nerimux::%copy-mode-cmd #\i)))
    (expect (eq :copy-mode-yank (nerimux::%copy-mode-cmd #\y)))
    (expect (eq :copy-mode-begin-selection (nerimux::%copy-mode-cmd #\Space))))

  ;; %copy-mode-cmd returns NIL when CH is NIL.
  (it "copy-mode-cmd-returns-nil-for-nil-char"
    (expect (null (nerimux::%copy-mode-cmd nil))))
  ...
  (it "copy-mode-cmd-falls-through-to-key-binding-for-unknown-char"
    ...
    (finishes (nerimux::%copy-mode-cmd #\d)
    ...
    (dolist (row '((:right nil  "forward: p0 active, swap right → p1 first")
      ...
      (destructuring-bind (dir select-p1 desc) row
        ...
        (with-two-pane-h-session (sess win p0 p1)
          ...
          (nerimux::%swap-active-pane sess dir)
    ...
    (dolist (c '((:h "horizontal :no-focus must not error even when pane is too small")
      ...
      (destructuring-bind (orient desc) c
        (with-fake-session (s :nwindows 1 :npanes 1)
          (finishes (nerimux::%cmd-split s orient :no-focus t) "~A" desc)))))
  ...
  (it "make-dispatch-named-table-builds-lookup-table"
    (let* ((specs (list (list :named-keyword :detach :named-names (list "detach"))
           ...
           (table (nerimux::%make-dispatch-named-table specs)))
  ...
  (it "dispatch-named-command-detach"
    (with-fake-session (s)
      (expect (eq :detach (nerimux::%dispatch-named-command s "detach")))
      (expect (eq :unknown-command (nerimux::%dispatch-named-command s "detach-client")))))
  ...
  (it "dispatch-named-command-list-sessions"
    (with-fake-session (s :nwindows 1)
      (let ((*overlay* nil)
        ...
        (nerimux::%dispatch-named-command s "list-sessions")
  ...
  (it "dispatch-named-command-copy-mode-enter"
    (with-fake-session (s)
      (nerimux::%dispatch-named-command s "copy-mode-enter")
      (expect (nerimux::%copy-mode-active-p s))
      (expect (eq :unknown-command (nerimux::%dispatch-named-command s "copy-mode")))))
  ...
  (it "dispatch-named-command-display-menu"
    (with-fake-session (s)
      (let ((*overlay* nil)
        ...
        (expect (null (nerimux::%dispatch-named-command s "display-menu")))
        (expect (not (null nerimux::*active-menu*)))
        (assert-overlay-active
         "%dispatch-named-command 'display-menu' must open an overlay")
        (expect (eq :unknown-command (nerimux::%dispatch-named-command s "menu"))))))
  ...
  (it "dispatch-prefix-command-copy-mode-y-yanks"
    ...
    (with-fake-session (s)
      ...
      (expect (nerimux::%copy-mode-active-p s))
      (finishes (nerimux::dispatch-prefix-command s (char-code #\y))
  ...
  (it "dispatch-prefix-command-copy-mode-search-prompts"
    (dolist (ch '(#\/ #\?))
      (with-fake-session (s)
        (nerimux::dispatch-command s :copy-mode-enter nil)
        (let ((*prompt* nil))
          (nerimux::dispatch-prefix-command s (char-code ch))
  ...
  (it "dispatch-select-layout-even-does-not-error"
    (with-two-pane-h-session (sess win p0 p1)
      ...
      (finishes (nerimux::dispatch-command sess :select-layout-even-h nil)
                ":select-layout-even-h must not signal an error"))
    (with-two-pane-v-session (sess win p0 p1)
      (expect (and win p0 p1))
      (finishes (nerimux::dispatch-command sess :select-layout-even-v nil)
  ...
  (it "dispatch-break-pane-on-single-pane-window-is-noop"
    (with-fake-session (s :nwindows 1 :npanes 1)
      (let ((nwindows-before (length (session-windows s))))
        (finishes (nerimux::dispatch-command s :break-pane nil)
  ...
  (it "dispatch-break-pane-on-two-pane-window-creates-new-window"
    (with-two-pane-h-session (sess win p0 p1)
      ...
      (let ((nwindows-before (length (session-windows sess))))
        ...
        (handler-case
            (progn
              (nerimux::dispatch-command sess :break-pane nil)
  ...
  (it "dispatch-join-pane-opens-prompt"
    (with-fake-session (s :nwindows 2)
      (let ((*prompt* nil))
        (nerimux::dispatch-command s :join-pane nil)
        (expect (prompt-active-p)))))
  ...
  (it "dispatch-move-window-prompt-opens-prompt"
    (with-fake-session (s :nwindows 2)
      (let ((*prompt* nil))
        (nerimux::dispatch-command s :move-window-prompt nil)
        (expect (prompt-active-p))
  ...
  (it "dispatch-menu-select-executes-selected-command"
    (with-fake-session (s)
      (let ((nerimux::*active-menu*
        ...
        (expect (eq :detach (nerimux::dispatch-command s :menu-select nil))))))
  ...
  (it "dispatch-menu-select-nil-menu-is-noop"
    (with-fake-session (s)
      (let ((nerimux::*active-menu* nil))
        (finishes (nerimux::dispatch-command s :menu-select nil)
    ...
    (dolist (key '(#\n #\p))
      (with-fake-session (s :nwindows 2)
        (let ((w1 (second (session-windows s))))
          (nerimux::dispatch-prefix-command s (char-code key))
  ...
  (it "dispatch-prefix-command-unknown-byte-is-noop"
    (with-fake-session (s)
      ...
      (finishes (nerimux::dispatch-prefix-command s 0)
