...
(describe "dispatch-suite"
  ...
  (it "dispatch-set-environment-empty-input-is-noop"
    (with-fake-session (s)
      (let ((*prompt* nil))
        (nerimux::dispatch-command s :set-environment nil)
  ...
  (it "cmd-set-environment-u-unsets-variable"
    (with-fake-session (s)
      (let ((name "NERIMUX_TEST_ENV_VAR_U"))
        (session-set-environment s name "hello")
        (nerimux::%cmd-set-environment-prompt s (list "-u" name))
  ...
  (it "cmd-set-environment-g-sets-process-variable"
    (with-fake-session (s)
      (let ((name "NERIMUX_TEST_ENV_VAR_G")
            (value "global value"))
        (with-temporary-posix-environment-variable (name nil)
          (nerimux::%cmd-set-environment-prompt s (list "-g" name value))
  ...
  (it "cmd-set-environment-f-expands-value-as-format"
    (with-fake-session (s)
      (let ((name "NERIMUX_TEST_ENV_VAR_F"))
        (nerimux::%cmd-set-environment-prompt s (list "-F" name "#{session_name}"))
        (multiple-value-bind (value source) (session-environment-value s name)
          (declare (ignore source))
          (expect (string= (nerimux::session-name s) value))
  ...
  (it "cmd-set-environment-t-target-session-writes-value"
    (with-fake-session (s)
      (let ((name "NERIMUX_TEST_ENV_VAR_T"))
          ...
          (let ((target (make-fake-session :nwindows 1 :npanes 1)))
            (with-session-and-window-names (target "beta")
              (with-registered-sessions (("alpha" s) ("beta" target))
                (nerimux::%cmd-set-environment-prompt s (list "-t" "beta" name "value"))
  ...
  (it "cmd-set-environment-g-and-t-are-mutually-exclusive"
    (with-fake-session (s)
      (let ((name "NERIMUX_TEST_ENV_VAR_GT"))
        (let ((nerimux::*overlay* nil))
          (nerimux::%cmd-set-environment-prompt s (list "-g" "-t" "ignored" name "value"))
          (expect (search "mutually exclusive" nerimux::*overlay*))))))
  ...
  (it "cmd-set-environment-unknown-flag-is-rejected-before-mutating"
    (with-fake-session (s)
      (let ((name "NERIMUX_TEST_ENV_VAR_UNKNOWN_FLAG"))
        ...
        (let ((*overlay* nil))
          (nerimux::%cmd-set-environment-prompt s (list "-Z" name "value"))
