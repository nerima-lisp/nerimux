...
(describe "dispatch-suite"
  ...
  (it "substitute-prompt-response-table"
    (dolist (row '(("rename-window '%%'" "shell" "rename-window 'shell'" "%% form")
      ...
      (destructuring-bind (template input expected desc) row
        (declare (ignore desc))
        (expect (string= expected (nerimux::%substitute-prompt-response template input))))))
  ...
  (it "command-prompt-template-without-p-substitutes-response"
    (with-fake-session (s)
      (let ((*prompt* nil))
        (nerimux::%run-command-line s "command-prompt \"set-option -g @cp 'X-%%'\"")
        (expect (prompt-active-p))
        (funcall (prompt-on-submit *prompt*) "foo")
        (expect (string= "X-foo" (nerimux/options:get-option "@cp"))))))
  ...
  (it "command-prompt-i-runs-template-on-each-edit"
    (with-fake-session (s)
      (let ((*prompt* nil))
        (nerimux::%run-command-line s "command-prompt -i \"set-option -g @inc '%1'\"")
        (expect (prompt-active-p))
        (expect (prompt-on-change *prompt*) :to-be-truthy)
        (funcall (prompt-on-change *prompt*) "ab")
        (expect (string= "ab" (nerimux/options:get-option "@inc")))
        (funcall (prompt-on-change *prompt*) "abc")
        (expect (string= "abc" (nerimux/options:get-option "@inc"))))))
  ...
  (it "command-prompt-N-accepts-digits-only"
    (with-fake-session (s)
      (let ((*prompt* nil))
        (nerimux::%run-command-line s "command-prompt -N \"select-window -t %1\"")
        (expect (prompt-active-p))
        (expect (nerimux/prompt:prompt-numeric-only *prompt*) :to-be-truthy)
        (nerimux/prompt:prompt-input #\a)
        (nerimux/prompt:prompt-input #\3)
        (expect (string= "3" (nerimux/prompt:prompt-buffer *prompt*))))))
  ...
  (it "command-prompt-F-expands-command-as-format"
    (with-fake-session (s)
      (let ((*prompt* nil))
        ...
        (let ((value (nerimux/options:get-option "@fmt")))
  ...
  (it "copy-mode-s-enters-on-source-pane-screen"
    (with-two-pane-h-session (s win p0 p1)
      (with-command-test-state (s :overlay t)
        (nerimux::%run-command-line s "copy-mode -s %2")
        (expect (nerimux/terminal/types:screen-copy-mode-p
                 (nerimux/model:pane-screen p1))
                :to-be-truthy)
        (expect (null (nerimux/terminal/types:screen-copy-mode-p
                       (nerimux/model:pane-screen p0)))))))

  ;; command-prompt -l requests a one-keypress prompt (like -1; nerimux's -1
  ;; already submits the untranslated character, which is -l's semantics).
  (it "command-prompt-l-is-single-key"
    (with-fake-session (s)
      (let ((*prompt* nil))
        (nerimux::%run-command-line s "command-prompt -l \"send-keys %1\"")
        (expect (prompt-active-p))
        (expect (nerimux/prompt:prompt-single-key *prompt*) :to-be-truthy))))
  ...
  (it "command-prompt-e-closes-on-focus-out"
    (with-fake-session (s)
      (let ((*prompt* nil)
            (nerimux::*dirty* nil))
        (nerimux::%run-command-line s "command-prompt -e")
        (expect (prompt-active-p))
        ;; Focus-in: prompt stays.
        (nerimux::%handle-escape-focus-change s (vector 27 91 73)) ; ESC [ I
        (expect (prompt-active-p))
        ;; Focus-out: prompt closes.
        (nerimux::%handle-escape-focus-change s (vector 27 91 79)) ; ESC [ O
  ...
  (it "command-prompt-without-e-survives-focus-out"
    (with-fake-session (s)
      (let ((*prompt* nil)
            (nerimux::*dirty* nil))
        (nerimux::%run-command-line s "command-prompt")
        (nerimux::%handle-escape-focus-change s (vector 27 91 79))
        (expect (prompt-active-p))))))
