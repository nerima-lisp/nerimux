...
(describe "dispatch-suite"
  ...
  (it "dispatch-synchronize-panes-toggles"
    (with-fake-session (s)
      (let ((*overlay* nil))
        ...
        (nerimux::dispatch-command s :synchronize-panes nil)
  ...
  (it "dispatch-lock-unlock-session"
    (with-fake-session (s)
      ...
      (nerimux::dispatch-command s :lock-session nil)
      (expect (session-locked-p s) :to-be-truthy)
      (nerimux::dispatch-command s :unlock-session nil)
      (expect (session-locked-p s) :to-be-falsy)))
  ...
  (it "dispatch-last-window-selects-previous-window"
    (with-fake-session (s :nwindows 2)
      (let* ((w0 (first  (session-windows s)))
        ...
        (nerimux::dispatch-command s :last-window nil)
  ...
  (it "dispatch-show-options-shows-overlay"
    (with-fake-session (s)
      (let ((*overlay* nil))
        (nerimux::dispatch-command s :show-options nil)
  ...
  (it "run-command-show-options-with-name-shows-overlay"
    (with-fake-session (s)
      (let ((*prompt* nil)
            (*overlay* nil))
        (nerimux::%run-command-line s "show-options status")
  ...
  (it "run-command-show-options-scope-flags"
    (with-fake-session (s)
      (let ((*overlay* nil))
        (nerimux::%run-command-line s "show-options -s escape-time")
        (assert-overlay-contains "escape-time" *overlay*
                                 "show-options -s <name>"))
      (let ((*overlay* nil))
        (nerimux::%run-command-line s "show-window-options mode-keys")
  ...
  (it "run-command-show-window-options-targets-window"
    (with-fake-session (s :nwindows 2)
      (let* ((windows (nerimux/model:session-windows s))
        ...
        (nerimux/options:set-option-for-window "mode-keys" "emacs" active)
        (nerimux/options:set-option-for-window "mode-keys" "vi" target)
        (let ((*overlay* nil))
          (nerimux::%run-command-line s "show-window-options -t work mode-keys")
  ...
  (it "run-command-show-options-quiet-and-value-only"
    (with-fake-session (s)
      (let ((*overlay* nil))
        (nerimux::%run-command-line s "show-options -q no-such-option")
        (expect (null *overlay*)))
      (let ((*overlay* nil))
        (nerimux::%run-command-line s "show-options -v status")
        (expect (and *overlay* (null (search "status" *overlay*)))))
      (let ((*overlay* nil))
        (nerimux::%run-command-line s "show-options -qv no-such-option")
        (expect (null *overlay*)))))
  ...
  (it "run-command-show-options-hooks-flag"
    (with-isolated-hooks
      ...
      (with-fake-session (s)
        (let ((*overlay* nil))
          (nerimux::%run-command-line s "show-options -H")
  ...
  (it "run-command-show-options-inherited-flag"
    (with-fake-session (s)
      (let ((*overlay* nil))
        (nerimux::%run-command-line s "show-options -A")
  ...
  (it "run-command-show-window-options-inherited-flag-marks-inherited"
    (with-isolated-config
      (with-fake-session (s)
        ...
        (let ((*overlay* nil))
          (nerimux::%run-command-line s "show-window-options -A mode-keys")
          (assert-overlay-contains "* mode-keys vi" *overlay*
  ...
  (it "dispatch-respawn-pane-does-not-error"
    (with-fake-session (s :nwindows 1 :npanes 1)
      (handler-case
          (progn
            (nerimux::dispatch-command s :respawn-pane nil)
            (expect t))
        (error (e)
          (declare (ignore e))
          (expect t)))))
  ...
  (it "dispatch-pipe-pane-opens-prompt-when-not-open"
    (with-fake-session (s)
      (let ((*prompt* nil))
        (nerimux::dispatch-command s :pipe-pane nil)
  ...
  (it "dispatch-last-pane-selects-previous-pane"
    (with-fake-two-pane-session (s)
      (let* ((win (session-active-window s))
        ...
        (nerimux::dispatch-command s :last-pane nil)
