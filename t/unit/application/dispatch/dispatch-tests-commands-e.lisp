...
(describe "dispatch-suite"
  ...
  (it "dispatch-switch-client-next-moves-to-next-session"
    (with-fake-session (s1 :nwindows 1)
      (let* ((s2  (make-fake-session :nwindows 1))
        ...
        (let ((nerimux::*server-sessions* reg))
          (nerimux::dispatch-command s1 :switch-client-next nil)
  ...
  (it "dispatch-switch-client-prev-does-not-error"
    (with-fake-session (s)
      (finishes (nerimux::dispatch-command s :switch-client-prev nil)
  ...
  (it "dispatch-last-session-does-not-error"
    (with-fake-session (s)
      (let ((nerimux::*server-sessions* (list (cons (session-name s) s))))
        (finishes (nerimux::dispatch-command s :last-session nil)
  ...
  (it "dispatch-new-session-does-not-error"
    (with-fake-session (s)
      (let ((nerimux::*server-sessions* nil))
        (finishes (nerimux::dispatch-command s :new-session nil)
    ...
    (with-fake-session (s)
      (let ((name (session-name s)))
        (let ((nerimux::*server-sessions* (list (cons name s))))
          (expect (eq :quit (nerimux::dispatch-command s :kill-session nil)))))))
        ...
        (let ((nerimux::*server-sessions* (list (cons name s))))
          (dolist (win (session-windows s))
            (setf (nerimux/model:window-activity-flag win) t
                  (nerimux/model:window-silence-flag  win) t))
          (nerimux::%cmd-kill-session-arg s '("-C"))
          (expect (nerimux::server-find-session name))
          (dolist (win (session-windows s))
            (expect (null (nerimux/model:window-activity-flag win)))
            (expect (null (nerimux/model:window-silence-flag win))))))))
  ...
  (it "dispatch-clear-mark-clears-server-marked-pane"
    (with-fake-session (s :nwindows 1 :npanes 1)
      (let ((ap (session-active-pane s)))
        (nerimux::dispatch-command s :mark-pane nil)
        (expect (pane-marked ap) :to-be-truthy)
        (nerimux::dispatch-command s :clear-mark nil)
        (expect (pane-marked ap) :to-be-falsy))))
  ...
  (it "dispatch-mark-pane-cross-window-clears-previous"
    (with-fake-session (s :nwindows 2)
      (let* ((win1 (first  (session-windows s)))
        ...
        (expect (eq p2 nerimux::*server-marked-pane*)))))
  ...
  (it "dispatch-next-layout-cycles-layout"
    (with-fake-session (s :nwindows 1 :npanes 1)
      (finishes (nerimux::dispatch-command s :next-layout nil)
  ...
  (it "dispatch-choose-client-shows-overlay"
    (with-fake-session (s)
      (let ((*overlay* nil))
        (nerimux::dispatch-command s :choose-client nil)
  ...
  (it "command-usage-table-omits-local-chooser-unsupported-args"
    (dolist (name '("choose-buffer" "choose-client" "choose-tree" "choose-window" "list-buffers"))
      (expect (string= "" (cdr (assoc name nerimux::*command-usage-table* :test #'string=))))))
  ...
  (it "dispatch-display-info-shows-overlay"
    (with-fake-session (s)
      (let ((*overlay* nil))
        (nerimux::dispatch-command s :display-info nil)
  ...
  (it "dispatch-list-buffers-no-buffers-shows-overlay"
    (with-fake-session (s)
      (let ((*overlay* nil)
        ...
        (nerimux::dispatch-command s :list-buffers nil)
  ...
  (it "dispatch-list-buffers-populated-shows-entries"
    (with-fake-session (s)
      (let ((*overlay* nil)
            (nerimux/buffer:*paste-buffers* (list (cons "buffer1" "hello")
                                                  (cons "buffer0" "world"))))
        (nerimux::dispatch-command s :list-buffers nil)
  ...
  (it "dispatch-show-buffer-shows-content"
    (with-fake-session (s)
      (let ((*overlay* nil)
            (nerimux/buffer:*paste-buffers* (list (cons "buffer0" "test-content"))))
        (nerimux::dispatch-command s :show-buffer nil)
  ...
  (it "dispatch-delete-buffer-removes-first-entry"
    (with-fake-session (s)
      (let ((nerimux/buffer:*paste-buffers* (list (cons "buffer0" "todelete"))))
        (nerimux::dispatch-command s :delete-buffer nil)
  ...
  (it "paste-buffer-text-translates-lf-to-cr-by-default"
    (dolist (c (list (list (format nil "a~Cb~Cc" #\Return #\Return)
      ...
      (destructuring-bind (expected input no-replace desc) c
        (declare (ignore desc))
        (expect (equal expected (nerimux::%paste-buffer-text input no-replace))))))
  ...
  (it "paste-buffer-text-separator-overrides-default"
    (dolist (c (list (list "a-b-c"               (format nil "a~%b~%c") nil "-"  "-s '-': LF -> '-'")
      ...
      (destructuring-bind (expected input no-replace sep desc) c
        (declare (ignore desc))
        (expect (string= expected (nerimux::%paste-buffer-text input no-replace sep))))))
  ...
  (it "dispatch-save-buffer-opens-prompt-when-buffer-exists"
    (with-fake-session (s)
      (let ((*prompt* nil)
            (nerimux/buffer:*paste-buffers* (list (cons "buffer0" "save-me"))))
        (nerimux::dispatch-command s :save-buffer nil)
        (expect (prompt-active-p)))))
    ...
    (with-fake-session (s)
      (let ((*prompt* nil) (*overlay* nil)
            (nerimux/buffer:*paste-buffers* (list (cons "buffer1" "alpha")
                                                  (cons "buffer0" "beta"))))
        (nerimux::dispatch-command s :choose-buffer nil)
  ...
  (it "signal-channel-prompt-opens-prompt"
    (with-loop-state
      (let ((*prompt* nil))
        (nerimux::%signal-channel-prompt "test-channel")
        (expect (prompt-active-p))
