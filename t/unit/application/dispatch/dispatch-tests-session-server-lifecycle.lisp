    ...
    (let ((nerimux::*running* t)
      ...
      (with-empty-session (s)
        ...
        (expect (not (eq :unknown-command
                         (nerimux::%dispatch-named-command s "start-server"))))
        ;; send-prefix: recognised; an empty session has no active pane -> safe no-op.
        (expect (not (eq :unknown-command
                         (nerimux::%dispatch-named-command s "send-prefix"))))
        ;; kill-server: recognised -> dispatches :kill-server, returns :quit and
        ;; clears *running*.
        (expect (eq :quit (nerimux::%dispatch-named-command s "kill-server")))
        ...
        (expect (eq :unknown-command
                    (nerimux::%dispatch-named-command s "definitely-not-a-command-xyz")))))))
