...
(describe "events-suite"
  ...
  (it "copy-mode-vi-table-binding-overrides-hardcoded"
    (with-isolated-config
      ...
      (nerimux/config:key-table-bind "copy-mode-vi" #\v :copy-mode-begin-selection)
  ...
  (define-copy-mode-table-selection-cases
    (copy-mode-key-table-selection-follows-mode-keys-vi
     ...
     ((nerimux/config:key-table-bind "copy-mode-vi" #\v :copy-mode-exit)
      (nerimux/config:key-table-bind "copy-mode" #\v :copy-mode-begin-selection))
     (nerimux::process-byte s (char-code #\v) state)
  ...
  (it "copy-mode-vi-default-hjkl-move-cursor"
    (with-copy-mode-vi-state (s screen state)
      ...
      (setf (nerimux/terminal:screen-copy-cursor screen) (cons 1 1))
      (nerimux::process-byte s (char-code #\j) state)
      (expect (equal (cons 2 1) (nerimux/terminal:screen-copy-cursor screen)))
      (nerimux::process-byte s (char-code #\k) state)
      (expect (equal (cons 1 1) (nerimux/terminal:screen-copy-cursor screen)))
      (nerimux::process-byte s (char-code #\l) state)
      (expect (equal (cons 1 2) (nerimux/terminal:screen-copy-cursor screen)))
      (nerimux::process-byte s (char-code #\h) state)
      (expect (equal (cons 1 1) (nerimux/terminal:screen-copy-cursor screen)))))
  ...
  (it "copy-mode-vi-percent-jumps-to-next-matching-bracket"
    (with-copy-mode-vi-state (s screen state)
      (dotimes (i 7)
        (setf (nerimux/terminal/types:screen-cell screen i 0)
              (nerimux/terminal/types:make-cell :char (char "(a(b)c)" i))))
      (setf (nerimux/terminal:screen-copy-cursor screen) (cons 0 0))
      (nerimux::process-byte s (char-code #\%) state)
      (expect (equal (cons 0 6) (nerimux/terminal:screen-copy-cursor screen)))))
    ...
    (with-copy-mode-vi-state (s screen state)
      (let ((text "xx a.b aXb a.b"))
        (dotimes (i (length text))
          (setf (nerimux/terminal/types:screen-cell screen i 0)
                (nerimux/terminal/types:make-cell :char (char text i)))))
      (setf (nerimux/terminal:screen-copy-cursor screen) (cons 0 3))
      (nerimux::process-byte s (char-code #\*) state)
      (expect (equal (cons 0 11) (nerimux/terminal:screen-copy-cursor screen)))
      (setf (nerimux/terminal:screen-copy-cursor screen) (cons 0 12))
      (nerimux::process-byte s (char-code #\#) state)
      (expect (equal (cons 0 11) (nerimux/terminal:screen-copy-cursor screen)))
      (expect (string= "a\\.b"
  ...
  (it "copy-mode-vi-named-special-bindings-fire"
    (dolist (row '(("PageUp" (27 91 53 126)     "copy-mode-vi PageUp binding must fire")
      ...
      (destructuring-bind (key bytes msg) row
        (declare (ignore msg))
        (with-copy-mode-vi-state (s screen state)
          (nerimux/config:key-table-bind "copy-mode-vi" key :copy-mode-exit)
  ...
  (define-copy-mode-table-selection-cases
    (copy-mode-pagedown-uses-emacs-copy-mode-key-table
     ...
     ((nerimux/config:key-table-bind "copy-mode-vi" "PageDown" :copy-mode-page-up)
      (nerimux/config:key-table-bind "copy-mode" "PageDown" :copy-mode-exit))
    ...
    (copy-mode-key-table-selection-follows-mode-keys-emacs
     ...
     (nerimux::process-byte s (char-code #\v) state)
     (expect (nerimux/terminal:screen-copy-mode-p screen))
     (expect (nerimux/terminal:screen-copy-selecting screen)))
    (copy-mode-meta-key-table-selection-follows-mode-keys-emacs
     "In emacs mode, ESC-prefixed Meta keys use the copy-mode table."
     with-copy-mode-emacs-state
     ((nerimux/config:key-table-bind "copy-mode-vi" "M-f" :copy-mode-exit)
      (nerimux/config:key-table-bind "copy-mode" "M-f" :copy-mode-begin-selection))
  ...
  (it "copy-mode-escape-control-key-does-not-fall-back-to-copy-mode-table"
    (with-copy-mode-emacs-state (s screen state)
      (nerimux/config:key-table-bind "copy-mode" "C-b" :copy-mode-begin-selection)
      (send-copy-mode-bytes s state '(27 2))
      (expect (nerimux/terminal:screen-copy-mode-p screen) :to-be-truthy)
      (expect (nerimux/terminal:screen-copy-selecting screen) :to-be-falsy))))
