...
;;;
;;; Moved verbatim from t/helpers-pty-runtime.lisp (R9.2): nerimux/test no
...
;;; with-pty-session (t/helpers-pty-runtime.lisp's fake-session + PTY-gate
...
;;; ── with-session (moved from t/helpers-session-fixtures.lisp) ───────────────
;;;
...
;;;
;;; nerimux/test's WITH-EMPTY-REGISTRY (t/helpers-input-fixtures.lisp) is a thin
...
(defmacro with-empty-registry (&body body)
  ...
  `(let ((nerimux::*server-sessions* nil)) ,@body))

;;; ── with-test-listener / %test-socket-path (duplicated) ──────────────────────
;;;
;;; nerimux/test's copies (t/helpers-network-listener.lisp) stayed put: they are
...
;;; ── drain-pty (moved from t/integration/pty-tests.lisp) ──────────────────────
;;;
...
;;;
;;; nerimux/test's copy (t/helpers-pane-fixtures.lisp) stayed put: it is used
