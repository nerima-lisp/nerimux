...
(define-command-rules %handle-client-ui-command (session conn cmd target args)
  ...
  ((%client-ui-mode-p cmd)
   (cond
     ...
     (t
      ...
      (%mark-dirty)))
   t)
  ((:enter-normal :enter-input :enter-copy :enter-command
    :cancel :accept :toggle-copy)
   (cond
     ((or (eq cmd :enter-copy)
          (and (eq cmd :toggle-copy)
               (not (eq (client-conn-modal conn) :scrollback))))
      (%client-enter-copy-mode session conn))
     ((and (eq cmd :toggle-copy)
