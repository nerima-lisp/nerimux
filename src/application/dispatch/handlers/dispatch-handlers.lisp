...
(defmacro define-show-options-handler (fn-name scope-label &optional scope-arg)
  "Generate a zero-argument function named FN-NAME that shows a scoped options
   ...
   SCOPE-ARG (a keyword) is forwarded to nerimux/options:show-options when
  ...
  `(defun ,fn-name ()
     ...
     (show-overlay
      (with-output-to-string (stream)
        ...
        ,(if scope-arg
             `(write-string (nerimux/options:show-options ,scope-arg) stream)
             `(write-string (nerimux/options:show-options) stream))))))
...
(define-command-handlers
  ...
  (:copy-mode-choose-buffer
   (show-built-overlay (stream)
     (write-string (%paste-buffer-listing-string
                    (nerimux/buffer:list-paste-buffers)
  ...
  (:paste-buffer
   (let* ((text (nerimux/buffer:get-paste-buffer))
  ...
  (:send-prefix
   ...
   (with-active-pane (ap session)
     (and (not *client-read-only*)
          (%send-byte-to-pane ap nerimux/config:*prefix-key-code*))))
  ...
  (:show-options
   (show-overlay (nerimux/options:show-options)))
