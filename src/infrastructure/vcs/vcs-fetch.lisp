...
(defun fetch-repository-async
    (repository &key on-complete on-error callback-dispatch)
    ...
    (if (%fetch-begin key)
        (first
         (refresh-repositories-async
          (list repository)
          :status-reader #'%read-fetched-repository-status
...
(defun fetch-organization-async
    (organization &key on-complete on-error callback-dispatch)
