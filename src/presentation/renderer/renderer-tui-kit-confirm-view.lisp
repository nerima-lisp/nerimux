...
(defun %confirm-view-content-lines (view)
  ...
  (append
   (mapcar #'%confirm-view-field-line (confirm-view-fields view))
   (list "")
...
(defun %draw-confirm-view-field (surface row col field width)
  (cl-tui-kit/core:surface-draw-styled-text
   ...
   (list (cl-tui-kit/core:make-text-span
          (format nil "~A: " (car field)) :style (%confirm-view-key-style))
         (cl-tui-kit/core:make-text-span (princ-to-string (cdr field))))
   :max-width width))
...
(defun %render-confirm-view-box (surface rectangle view)
  ...
  (let ((box (cl-tui-kit/widgets:make-box-widget
              ...
              :id :nerimux-confirm-box :border-kind :single)))
  ...
  (let* ((inner (%box-widget-inner-rectangle rectangle))
    ...
    (cl-tui-kit/core:surface-draw-text
     surface col row (%confirm-view-prompt-line view) :max-width width)))
...
(defun %stamp-confirm-view-title (surface rectangle title)
  (let* ((inner-width (max 0 (- (cl-tui-kit/core:rectangle-width rectangle) 4)))
    ...
    (cl-tui-kit/core:surface-draw-text
     ...
     text
     :style (%confirm-view-heading-style))))
