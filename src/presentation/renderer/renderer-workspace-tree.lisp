...
;;;;
;;;; Overview redesign PR2: the tree is now the overview's only panel (the
;;;; WORKTREES/PANES/PREVIEW three-column layout is gone -- see
;;;; renderer-workspace.lisp) and defaults to fully expanded rather than
;;;; collapsed, so this file's collapse table inverts sense (see
;;;; %WORKSPACE-NODE-EXPANDED-P) and gains a text filter and a worktree-row
;;;; info cluster.
...
(defun %workspace-tree-node-search-text (kind object)
...
(defun %workspace-tree-node-matches-filter-p (kind object downcased-filter)
  ...
  (and downcased-filter
       ...
       (search downcased-filter (%workspace-tree-node-search-text kind object))
       t))
        ...
        (dotimes (index count)
          (let* ((entry (aref vector index))
            ...
            (when (%workspace-tree-node-matches-filter-p
                   (fourth entry) (third entry) downcased-filter)
...
(defun workspace-tree-objects (organizations collapsed-node-ids &key filter)
...
(defun %workspace-flat-tree-entries
    (organizations collapsed-node-ids &key refreshing-ids stale-ids filter)
  "Flatten ORGANIZATIONS into (LEVEL LABEL OBJECT KIND) display tuples,
   optionally narrowed to FILTER (see %WORKSPACE-FILTER-TREE-ENTRIES). A
   worktree's window row (level 3) is only emitted when it has 2 or more
   windows; with exactly one window (the common case), that window
   contributes no row of its own and its panes attach directly under the
   worktree at level 3 instead of level 4 -- collapsing the one-window,
   one-level-of-nothing-to-show case out of the tree entirely.

   Search penetrates collapse: when FILTER is non-blank, every organization
   and repository is descended into regardless of COLLAPSED-NODE-IDS -- a
   collapsed repository can still hold the worktree the user is searching
   for, and the contract a caller reads off this function's name is \"search
   the whole tree\", not \"search whatever happens to be expanded\". The
   COLLAPSED-NODE-IDS gates below are skipped entirely in that case, and
   %WORKSPACE-FILTER-TREE-ENTRIES alone decides what is actually visible
   from the (now fully descended) raw entries."
  (let ((filter-active-p
    ...
    (dolist (organization organizations)
      ...
      (when (or filter-active-p
                (%workspace-node-expanded-p
                 :organization (organization-id organization) collapsed-node-ids))
        (dolist (repository (organization-repositories organization))
          ...
          (when (or filter-active-p
                    (%workspace-node-expanded-p
                     :repository (repository-id repository) collapsed-node-ids))
            (dolist (worktree (repository-worktrees repository))
              (push (list 2
                          ...
                          worktree :worktree)
                    entries)
              (let ((windows (%worktree-tree-windows worktree)))
                (if (>= (length windows) 2)
                    ...
                    (dolist (window windows)
                      (dolist (pane (window-panes window))
                        (push (list 3 (%pane-tree-label pane) pane :pane)
                              entries))))))))))
    (%workspace-filter-tree-entries (nreverse entries) filter)))
...
(defun workspace-tree-view-rows (terminal-rows)
  "Rows available for the tree in the one-column overview layout:
   ...
   footer(1), floored at 1. Shared with the bootstrap scroll-clamping math
   for the same reason %WORKSPACE-LEFT-WIDTH used to be shared: two layers
   computing this independently can silently disagree about where the tree
   ends."
  (max 1 (- terminal-rows 6)))
