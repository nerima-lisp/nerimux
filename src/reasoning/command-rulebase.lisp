...
(defun %command-rules ()
  ...
  (list
   (cl-prolog-kit:make-clause '(scriptable ?name)
                          (list '(usage ?name "")))))

(defun build-command-rulebase (usage-pairs)
  "Build a cl-prolog-kit rulebase from USAGE-PAIRS (a NAME . USAGE alist)."
  (let ((clauses '()))
    (dolist (pair usage-pairs)
      (destructuring-bind (name . usage) pair
        (push (cl-prolog-kit:make-clause (list 'command name)) clauses)
        (push (cl-prolog-kit:make-clause (list 'usage name usage)) clauses)
        (dolist (flag (%parse-usage-flags usage))
          (push (cl-prolog-kit:make-clause (list 'accepts-flag name flag)) clauses))))
...
(defun command-accepts-flag-p (rulebase name flag)
  ...
  (cl-prolog-kit:prolog-succeeds-p rulebase (list 'accepts-flag name flag)))

;;; NOTE: these three deliberately do NOT use cl-prolog-kit's SETOF/3, despite it
;;; looking like a natural fit — command/flag names in this domain are raw
;;; Lisp strings, and cl-prolog-kit's standard-order-of-terms comparator (which
