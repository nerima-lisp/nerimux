...
(defun make-test-pane (w h &key (id 1) (content "") (x 0) (y 0))
  ...
  (let* ((screen (make-screen w h))
         (pane
          (make-pane :id id :x x :y y :width w :height h :fd -1 :screen screen)))
    (unless (string= content "")
      (feed screen content))
    pane))

(defun make-selecting-screen (w h
                                mark-row
                                mark-col
                                cursor-row
                                cursor-col
  ...
  (let ((screen (make-screen w h)))
    (setf (nerimux/terminal/types:screen-copy-mode-p screen) t
          ...
          (nerimux/terminal/types:screen-copy-mark screen) (cons mark-row
                                                                 mark-col)
          (nerimux/terminal/types:screen-copy-cursor screen) (cons cursor-row
                                                                   cursor-col)
...
(defun make-renderer-test-session (w h &key (content ""))
  ...
  (let* ((screen (make-screen w h))
         (pane
          (make-pane :id 1 :x 0 :y 0 :width w :height h :fd -1 :screen screen))
         (win
          (make-window :id 1 :name "1" :width w :height h :panes (list pane)))
...
(defun make-two-window-session (w h &key (w0-content "") (w1-content ""))
  ...
  (let* ((screen0 (make-screen w h))
         (pane0
          (make-pane :id 1 :x 0 :y 0 :width w :height h :fd -1 :screen screen0))
         (win0
          (make-window :id
                       1
                       :name
         ...
         (pane1
          (make-pane :id 2 :x 0 :y 0 :width w :height h :fd -1 :screen screen1))
         (win1
          (make-window :id
                       2
                       :name
    ...
    (unless (string= w0-content "")
      (feed screen0 w0-content))
    (unless (string= w1-content "")
      (feed screen1 w1-content))
