...
;;; :to-contain-sgr code).
(cl-weave:defmatcher :to-contain-sgr
                     (actual expected)
   ...
   \"48;2;10;20;30\")."
                     (and
                      (search (format nil "~C[~Am" #\Escape (first expected))
                              actual)
                      t))
...
(defun %bel-before-title-osc (frame)
  ...
  (let ((osc-start (search (format nil "~C]0;" #\Escape) frame)))
    (if osc-start
        (subseq frame 0 osc-start)
        frame)))

(defmacro with-copy-mode-render-fixture ((session-var pane-var
                                                      screen-var
                                                      w
                                                      h
                                                      &key
  ...
  `(let* ((,session-var (make-renderer-test-session ,w ,h :content ,content))
          (,pane-var
           (first (window-panes (session-active-window ,session-var))))
          (,screen-var (pane-screen ,pane-var)))
     ,@body))

(defmacro with-copy-mode-selection-fixture ((session-var pane-var
                                                         screen-var
                                                         w
                                                         h
                                                         &key
  ...
  `(with-copy-mode-render-fixture
    (,session-var ,pane-var ,screen-var ,w ,h :content ,content)
    (setf (screen-copy-mode-p ,screen-var) ,copy-mode-p
          ...
          (screen-copy-mark ,screen-var) (and ,mark-row
                                              ,mark-col
                                              (cons ,mark-row ,mark-col))
          (screen-copy-cursor ,screen-var) (and ,cursor-row
                                                ,cursor-col
                                                (cons ,cursor-row ,cursor-col)))
    ,@body))

(defun render-status-bar-output (sess rows
                                      cols
                                      &key
                                      ((:status-row status-row) nil
                                                                status-row-supplied-p))
  "Render the status bar for SESS to a string using the production renderer."
  (with-output-to-string (s)
    (if status-row-supplied-p
        (nerimux/renderer::render-status-bar s
                                             sess
                                             rows
                                             cols
                                             :status-row
    ...
    (mapcar
     (lambda (row)
       (destructuring-bind (input max expected desc) row
         (list (nerimux/renderer::%visible-truncate input max) expected desc)))
     ,cases)
    :test
    #'string=))
