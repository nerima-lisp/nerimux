                ...
                do (if (and (char= (char str i) #\#)
                       ...
                       (let ((close (position #\] str :start (+ i 2))))
                         (if close
                             (progn
                               (write-string
                                (%status-style-block-sgr
                                 (subseq str (+ i 2) close)
                                 base-sgr)
                                out)
                               (setf i (1+ close)))
                             (progn
  ...
  (let ((align nil)
    ...
    (dolist (a (%split-comma-attrs body))
      (let ((at (string-trim " " a)))
        (cond
          ((member at '("align=left" "align=l") :test #'string-equal)
           (setf align :left))
          ((member at
                   '("align=centre" "align=center" "align=c")
                   :test
                   #'string-equal)
           (setf align :centre))
          ((member at '("align=right" "align=r") :test #'string-equal)
           (setf align :right))
          ((plusp (length at)) (push at rest)))))
    (values align
            (when rest
              (format nil "~{~A~^,~}" (nreverse rest))))))
  ...
  (let ((close (position #\] raw :start (+ i 2))))
    (if close
        (multiple-value-bind (align rest) 
            (%split-align-attr (subseq raw (+ i 2) close))
          (if align
              (progn
                (when rest
                  (format (getf buckets align) "#[~A]" rest))
                (values (1+ close) align))
              (progn
                (write-string (subseq raw i (1+ close)) (getf buckets current))
                (values (1+ close) current))))
        (progn
          (write-char (char raw i) (getf buckets current))
          (values (1+ i) current)))))
...
(defun %status-align-buckets (raw)
  ...
  (let ((buckets
        ...
        (len (length raw)))
    (loop while (< i len)
          do (multiple-value-setq (i current) (%status-align-step raw
                                                                  i
                                                                  buckets
                                                                  current)))
      ...
      (%visible-truncate
       (with-output-to-string (out)
         (let ((col 0))
           (write-string l out)
           (incf col lw)
           (setf col (%status-emit-segment out
                                           col
                                           cols
                                           c
                                           cw
