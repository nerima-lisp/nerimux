...
(describe "global picker"
          (it
           "builds a deterministic organization repository worktree hierarchy"
           (let* ((organization
             ...
             (let* ((items
               ...
               (expect
                (= 1
                   (length
                    (nerimux/picker:filter-global-picker-items items "feature"))))
               ...
               (expect
                (eq attention-item
                    (nerimux/picker:select-global-picker-item items
                                                              (nerimux/picker:picker-item-id
                                                               attention-item)))))))
          ...
          (it "uses fallback labels for incomplete hierarchy data"
              (let* ((host-only
                     ...
                     (pane (nerimux/pane:make-pane :id 8)))
                (expect (string= "" (nerimux/picker::%picker-string nil)))
                (expect
                 (string= "text" (nerimux/picker::%picker-string "text")))
                ...
                (expect
                 (string= (namestring #P"picker-path")
                          (nerimux/picker::%first-picker-string ""
                                                                nil
                                                                #P"picker-path")))
                (expect
                 (string= "" (nerimux/picker::%first-picker-string "" nil nil)))
                (expect
                 (string= "github.com"
                ...
                (expect
                 (string= "github.com/team/repository"
                          (nerimux/picker::%repository-label
                           specification-repository)))
                (expect
                 (string= (namestring #P"local/repository")
                          (nerimux/picker::%repository-label
                           local-path-repository)))
                (expect
                ...
                (let* ((items
                  ...
                  (expect
                   (eq (first items)
                       (nerimux/picker:select-global-picker-item items
                                                                 (nerimux/picker:picker-item-label
                                                                  (first items)))))
                  (expect
                   (null (nerimux/picker:select-global-picker-item items 0)))
                  (expect
                   (null (nerimux/picker:select-global-picker-item items -1)))
                  (expect
                   (null (nerimux/picker:select-global-picker-item items 99)))
                  (expect
                   (null
                    (nerimux/picker:select-global-picker-item items "unknown")))
                  (expect
                   (null (nerimux/picker:select-global-picker-item items nil))))))
          (it
           "propagates repository, organization, worktree, and pane attention"
           (let* ((clean-repository
             ...
             (dolist 
               ...
               (expect
                (nerimux/picker:picker-item-attention-p
                 (nerimux/picker::%make-picker-item :id
             ...
             (expect
              (not
               (nerimux/picker:picker-item-attention-p
                (nerimux/picker::%make-picker-item :id "unknown" :kind :unknown))))))
              ...
              (let* ((organization
                     ...
                     (repository
                      (nerimux/workspace-model:make-repository :id
                                                               ...
                                                               organization
                                                               :specification
                                                               "github.com/metadata/repository"
                                                               :remote
                                                               "origin"
                                                               :local-path
                                                               #P"metadata/repository"
                                                               :conflict-p
                                                               t))
                     (worktree
                      (nerimux/workspace-model:make-worktree :id
                ...
                (let* ((items
                        (nerimux/picker:build-global-picker-items
                         (list organization)))
                       (focused-time (nerimux/pane:pane-last-focused-time pane))
                       (output-time (nerimux/pane:pane-last-output-time pane)))
                  (dolist 
                      (query
                       '("missing" "attention 3"
                                   "repositories 1 worktrees 1"
                                   "origin"
          ...
          (it "validates benchmark distributions and accepts empty scales"
              (signals error
                       (nerimux/picker:benchmark-global-picker
                        ...
                        -1))
              (signals error
                       (nerimux/picker:benchmark-global-picker :repository-count
                                                               -1))
              (signals error
                       (nerimux/picker:benchmark-global-picker :pane-count -1))
              (signals error
                       (nerimux/picker:benchmark-global-picker :worktree-count
                                                               -1))
              (signals error
                       (nerimux/picker:benchmark-global-picker :query nil))
              (signals error
                       (nerimux/picker:benchmark-global-picker
                        :organization-count
                        0
              ...
              (signals error
                       (nerimux/picker:benchmark-global-picker
                        ...
                        1))
              (let ((result
                     (nerimux/picker:benchmark-global-picker :organization-count
                                                             0
                                                             :repository-count
                                                             0
                                                             :worktree-count
                ...
                (let* ((items
                       ...
                       (pane-item
                        (find pane
                              items
                              :key
                              #'nerimux/picker:picker-item-pane
                              :test
                              #'eq)))
                  (expect (= 4 (length items)))
                  (expect
          ...
          (it
           "keeps the mandatory picker scale within the initial-render budget"
           (let ((result
                  (nerimux/picker:benchmark-global-picker :organization-count
                                                          1000
                                                          :repository-count
