      ...
      (let ((new (make-hash-table :test #'eql)))
        (maphash
         (lambda (y v)
           ...
           (cond
             ((and (> y top) (<= y bottom))
              (setf (gethash (1- y) new) t))
             ((or (< y top) (> y bottom))
              (setf (gethash y new) t))))
         ht)
...
(defun %palette-override-get (screen index)
  ...
  (let ((overrides (screen-palette-overrides screen)))
    (and overrides (<= 0 index 255) (svref overrides index))))
...
(defun %palette-override-set (screen index rgb)
  ...
  (when (<= 0 index 255)
    (let ((overrides
           (or (screen-palette-overrides screen)
               (setf (screen-palette-overrides screen) (make-array 256
                                                                   :initial-element
                                                                   nil)))))
