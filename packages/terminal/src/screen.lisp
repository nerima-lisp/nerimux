...
positions."
  (make-array cell-count
              :initial-contents
              (loop repeat cell-count
                    collect (blank-cell))))
...
wired after all packages have loaded."
  (let ((screen
         (%make-screen :width
                       ...
                       height
                       :cells
                       (%make-blank-cells (* width height))
                       :scroll-bottom
                       (1- height)
                       :lock
                       (make-lock :name "screen"))))
    (setf (screen-parser screen) (lambda (s byte)
                                   (nerimux/terminal/parser:ground-state s byte)))
    screen))

(defun screen-cell (screen x y)
  "Return the cell at column X, row Y."
  (aref (screen-cells screen) (+ (* y (screen-width screen)) x)))
