...
(describe "terminal-suite/parser-inline-predicates"
  ...
  (it "utf8-continuation-p-identifies-continuation-bytes"
    ...
    (expect (nerimux/terminal/parser::utf8-continuation-p #xBF) :to-be-truthy)
    (expect (nerimux/terminal/parser::utf8-continuation-p #x41) :to-be-falsy)
    (expect (nerimux/terminal/parser::utf8-continuation-p #xC0) :to-be-falsy))
  ...
  (it "utf8-lead-decode-returns-initial-accumulators"
    (dolist (row '((#xC2 2 1 "2-byte leader")
      ...
      (destructuring-bind (byte expected-acc expected-left desc) row
        (declare (ignore desc))
        (multiple-value-bind (acc left) (nerimux/terminal/parser::utf8-lead-decode byte)
