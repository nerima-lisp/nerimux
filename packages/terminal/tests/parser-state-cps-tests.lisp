...
(describe "terminal-suite/direct-parser-cps-suite"
  ...
  (it "ground-state-printable-writes-and-stays-ground"
    (with-screen (s 10 5)
      (let ((next (nerimux/terminal/parser:ground-state s 65)))
  ...
  (it "charset-designator-always-returns-ground-state"
    (with-screen (s 10 5)
      (expect (eq #'nerimux/terminal/parser:ground-state
              (funcall (nerimux/terminal/parser:make-charset-designator-k :g0) s 66)))
      (expect (eq #'nerimux/terminal/parser:ground-state
              (funcall (nerimux/terminal/parser:make-charset-designator-k :g0) s 48)))))
  ...
  (it "g1-line-drawing-via-so-remaps-characters"
    (with-screen (s 10 5)
      (feed s (format nil "~C)0~Cq" #\Escape (code-char #x0E)))
  ...
  (it "osc-state-other-bytes-stay-in-osc"
    (with-screen (s 10 5)
      (let ((k65 (nerimux/terminal/parser:osc-state s 65)))
