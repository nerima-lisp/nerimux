...
(defun round-trip-frame (frame)
  ...
  (with-temp-octet-file (path)
                        ...
                        (with-open-file 
                            (in path :element-type '(unsigned-byte 8))
                          (read-frame in))))
...
(defmacro with-connected-sockets ((path listener-var client-var conn-var) &body
                                                                          body)
