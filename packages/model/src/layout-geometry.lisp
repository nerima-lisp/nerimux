...
(defun %resize-find-split-climb (tree node orient)
  ...
  (multiple-value-bind (parent which) (layout-find-parent tree node)
    (cond
      ((null parent) (values nil nil))
      ((eq (layout-split-orientation parent) orient) (values parent which))
...
(defun split-child-geometry (pane orient)
  ...
  (orient-case orient
               :v
               (let* ((available-rows (- (pane-height pane) 1))
                      (first-rows (floor available-rows 2)))
                 (values (pane-x pane)
                         (+ (pane-y pane) first-rows 1)
                         (pane-width pane)
                         (- available-rows first-rows)))
               :h
               (let* ((available-cols (- (pane-width pane) 1))
                      (first-cols (floor available-cols 2)))
                 (values (+ (pane-x pane) first-cols 1)
                         (pane-y pane)
                         (- available-cols first-cols)
                         (pane-height pane)))))
