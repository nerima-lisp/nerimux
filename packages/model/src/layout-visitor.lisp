...
(define-layout-visitor layout-leaves
                       (node)
                       :docstring
                       "Collect every pane in NODE's subtree, left/top-to-right/bottom."
                       :on-null
                       nil
                       :on-leaf
                       (list pane)
                       :on-split
                       (append (layout-leaves first) (layout-leaves second)))

(defmacro define-layout-fold (name (node-var &rest extra-vars)
                                   &key
                                   on-null
                                   on-leaf
                                   on-split
...
(define-layout-fold layout-min-extent
                    (node orient)
                    :docstring
                    "Minimum cells NODE requires along ORIENT's split axis."
                    :on-null
                    0
                    :on-leaf
                    (%axis-floor orient)
                    :on-split
                    (let ((first-extent (layout-min-extent split-first orient))
                          (second-extent
                           (layout-min-extent split-second orient)))
                      (if (eq split-orient orient)
                          (+ first-extent 1 second-extent)
                          (max first-extent second-extent))))

(define-layout-fold layout-find-leaf
                    (node pane)
                    :docstring
                    ...
                    nil
                    :on-leaf
                    (when (eq leaf-pane pane)
                      node)
                    :on-split
                    (or (layout-find-leaf split-first pane)
